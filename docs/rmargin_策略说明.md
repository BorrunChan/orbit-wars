# `rmargin`（Kaggle ~1245）策略实现 + 算法说明

> **命名**:`rmargin`（reinforcement **margin**,增援余量版)= 我们当前最优。解耦自竞品 `orbit-wars-reyhan-ksatria.ipynb`(公开榜前 3 衍生),落地于 `external/reyhan/`(`main.py` + `params.json` + `orbit_lite/`),参数化引擎副本 `search/agent_main.py`。
> 提交 ref **53704199**,Kaggle **public ~1245**(1241–1247 波动,80 局)—— **我们最高的提交**,较自家 producer 系(exp48/expand ~1177 / baseline ~1180)**+65**。
> 衍生变体 **`rmargin-hoard`**(ref 53730874 = rmargin + 显式屯兵旋钮)= **1245,与 rmargin 持平、无增益**(见 §2 末)。
> 引擎与 `single-size`/`exp48` 同源(同一 `orbit_lite`)。算法本体见 `docs/single-size算法详解.md` / `docs/最优agent算法详解.md`,本文只讲 `rmargin` 在其上的**增量**与**为何更强**。
...
`rmargin` 的 `main.py` = 我们的 `exp48`(Producer:`orbit_lite` 向量化 sim-greedy + flow-diff 打分 + safe_drain + 后勤 regroup + **multi-size 候选** + **endgame 相位**)**加三层**:
...
- **这是 `orbit_lite` 早就内置但 single-size/exp48 没启用的特性**(`capture_floor(reinforcement=...)` + `reinforcement_timing_factor` 库里都有)。`rmargin` 把它接上了。
...
实测(h2h:`rmargin` config vs 我们):**`rmargin` 的配置自然屯出巨型 garrison** —— 一局 2P 跑满 500 步,reyhan 终局 **23 星 / 22428 兵**(对手被压到个位)。即:

- **`reinforce_size_beta` + 低 min_ships + 收紧后勤 + 细 size ⇒ 少打亏本仗、少浪费兵 ⇒ 产能堆成大舰队 ⇒ 长局靠兵力计分碾压。**
- 这与榜首(Isaiah 1790:27 星 / **32117 兵**)同向 —— **屯兵是赢长局的关键**,`rmargin` 用"调参 + 增援余量"**涌现式**地实现了一部分(不是显式屯兵规则)。
- 对比我们的 exp48/single-size:safe_drain 把星球清空、后勤更松、无增援余量 ⇒ 频繁亏本/浪费 ⇒ 站场兵力低 ⇒ 长局输。这是卡 ~1170 的根因。

**`rmargin-hoard` 实测(显式屯兵 = 死路)**:在 `rmargin` 之上加"owned≥N 星就抬 roi 停手"的显式屯兵旋钮(ref 53730874)。猛档(roi×3/min10)**backfire**(像 v111 冻死扩张:一局只剩 1 星/140 兵);温和档(roi×1.5~2)**no-op**(本地 8/8/4、Kaggle **1245 = rmargin 持平**)。结论:**`rmargin` 的"屯"是 config 层"少浪费"自然涌现的,显式"停手攒兵"加不动**。方向应是**继续降低兵力浪费(增援余量、收紧后勤、精确 size)**,而非"到点停止扩张"。

---

## 2.5 ⚠️ 真实输局分析(2026-06-17,86 局回放)—— 推翻 §2 的"屯兵长局"叙事

拉 1239(ref 53704199)86 局真实回放,按**终局**算分拆输局:**2P 61%(23/38)、4P 42%(20/48)**(优于历史均值,确认 1239 最强)。但**输法不是 §2 说的"拖满 500 步被屯兵者兵力计分压死"**:

| 格式 | 赢家风格 | n | 步数 | 我星→敌星 |
|---|---|---|---|---|
| 2P | precise_structured | 6 | **158** | **0** → 20 |
| 2P | moderate_pressure | 6 | 291 | 2 → 20 |
| 2P | passive_turtle | 2 | 327 | 1.5 → 5.5 |
| 4P | precise_structured | 11 | **157** | **0** → 19.6 |
| 4P | moderate_pressure | 9 | **162** | **0** → 22.7 |
| 4P | passive_turtle | 7 | 275 | 0.3 → 8.3 |

- **主力输法(74%)= 第 150–160 步被打到 0 星、提前出局**(precise_structured + moderate_pressure)。根本到不了屯兵终局。
- §2 的"屯兵长局 tiebreak 输"只是 **passive_turtle(9)+ swarm(1)**的少数派。
- **结论:真正软肋是开局/中期被精确+施压型对手抢扩张抢崩团灭,不是"屯得不够"。** 这解释了为什么"更屯/更防"变体(thrift 751 / def0.5 1138 / hoard 1196)全部低于 rmargin —— 治错了病。方向应是**中期不被抢崩**(开局扩张速度 + 被攻不丢光),而非加深 hoard。

**本地无判别力实锤**:把 1239 起所有提交解包成对手集 `search/gauntlet/`(7 个真实跑分件)+ `search/gauntlet.py`,实测 **reyhan(1239)与 thrift(751)本地战表逐格完全相同**(2P 固定总盘零和对称,双位平均每方恒得一半)。连 proto1000 也分不出。→ **本地 h2h 对排名零价值,gauntlet 只当崩溃冒烟筛,Kaggle A/B vs 1239 是唯一判据。**

## 3. 距榜首(1790)还差什么

`rmargin` ~1245 仍比 Isaiah 1790 低 ~545。调参 + 增援余量拿到 +65,**剩下的大头仍是更彻底的屯兵 / 更优 control**(榜首 32k 兵 vs `rmargin` 22k vs 朴素 producer 几百)。下一步杠杆应继续往"减少兵力浪费、把产能更多转成站场兵力"挖,但**避免硬停扩张**。
...
.venv/bin/kaggle competitions submit -c orbit-wars -f external/reyhan/submission_reyhan.tar.gz -m "rmargin ~1245"
