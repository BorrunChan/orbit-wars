...
class ProducerLiteConfig:
    ...
    # --- comet grab: top players actively take comets (low prod=1 but free
    # expansion/卡位). default producer skips them (roi too low). boost their
    # estimated prod so the planner grabs them. ----------------------------
...
def plan_lite_waves(
    *,
    ...
    # --- comet 跳板: 占领移动彗星当移动前进基地. 顶尖42%占用率、占领后84%从彗星发兵
    # 投送(把兵运到对岸够远端目标). 两步: (1)boost 彗星估值让它过 roi 门槛 (2)强制彗星
    # 进 target 绕过 proximity top-K 过滤(彗星远/移动否则不进候选). 占领后彗星成 owned
    # source, producer 的 attack/regroup 自动从它发兵(跳板自然发生). ---
    _comet_idx = None
    if bool(getattr(config, "enable_comet", False)):
        ...
        if cids is not None and cids.numel() > 0:
            ...
            if cids.numel() > 0:
                pl_ids = obs_tensors["planets"][..., 0].long().reshape(-1)  # [P]
    ...
    # 强制可占彗星进 target (绕过 proximity top-K, 让 planner 拦截移动彗星)
