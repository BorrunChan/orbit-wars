...
class ProducerLiteConfig:
    ...
    # 欲擒故纵 opportunist (4P): boost capturing strong opponents' low-garrison planets
...
def _apply_opportunist_boost(*, score, obs, target_idx, cand_tgt_short, prod, player_count, config):
    """欲擒故纵 (多人感知): boost capturing any opponent-at-least-as-strong-as-me's
    low-garrison planets when they over-extend. (ported from agent_main hyb family)"""
    ...
    *,
    ...
    if bool(getattr(config, "enable_opportunist", False)):
        score = _apply_opportunist_boost(
            ...
            player_count=int(player_count), config=config,
        )
...
CONFIG_4P = replace(
    ...
    enable_opportunist=True,
    opportunist_max_garrison=15.0,
    opportunist_boost=1.5,
)
