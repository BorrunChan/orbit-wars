    ...
    *,
    ...
    if bool(getattr(config, "enable_opportunist", False)):
        score = _apply_opportunist_boost(
            score=score, obs=obs, target_idx=target_idx, cand_tgt_short=cand_tgt_short, config=config,
        )
...
def _build_defense_entries(
    *,
    ...
    # ------------------------------------------------------------------
    # Step 1: owned planets projected to FALL within the horizon. garrison_status
    # .ships is ABSOLUTE >=0 so the old `ships_at_H < 0` test NEVER fired (dead
    # code from i-the-orbit). Use owner-flip. (fixed 2026-06-20)
    Hreq = int(config.defense_threat_horizon)
    ...
    if getattr(status, "owner", None) is not None:
        falls = status.owner[:, 1:] != pid                        # [P, H] owner-flip
    else:
        falls = status.ships[:, 1:] <= 0.5
    threatened = owned & falls.any(dim=-1)
    if not bool(threatened.any()):
        return _empty_entries(device, dtype)
    fall_step = falls.float().argmax(dim=-1) + 1                   # first fall step [P]

    # ------------------------------------------------------------------
    # Step 2: reinforcement-aware enemy deficit (arrivals_by_owner) → nearest
    # in-time surplus source. Build LaunchEntries WITH angle — make_launch_set's
    # LaunchSet (no angle) crashes the downstream concat.
    ...
    de_src = []; de_dst = []; de_ships = []
    for t in tgt_indices.tolist():
        if len(de_src) >= int(config.defense_max_waves):
            break
        fs = int(fall_step[t].item())
        if abo is not None:
            inc = float(abo[t, 1:fs + 1, :].sum().item() - abo[t, 1:fs + 1, pid].sum().item())
        else:
            inc = float(ships_f[t].item()) + 1.0
        deficit = inc - float(ships_f[t].item())
        if deficit <= 0:
            continue
        need = deficit * float(config.defense_min_intercept_margin)
        srcs = owned.clone(); srcs[t] = False
        si = srcs.nonzero(as_tuple=False).squeeze(1)
        if si.numel() == 0:
            continue
        sp = fleet_speed(ships_f[si].clamp(min=1.0))
        eta = (d0[si, t] / sp.clamp(min=1e-6)).ceil()
        surplus = ships_f[si] - float(config.min_ships_to_launch)
        valid = (eta < float(fs)) & (surplus > 0)
        if not bool(valid.any()):
            continue
        best = int(torch.where(valid, d0[si, t], torch.full_like(d0[si, t], 1e9)).argmin().item())
        s = int(si[best].item())
        send = min(float(ships_f[s].item()) * 0.6, need + float(config.min_ships_to_launch))
        send = max(send, float(config.min_ships_to_launch))
        send = float(int(send))   # Kaggle requires integer-valued ship counts
        de_src.append(s); de_dst.append(t); de_ships.append(send)

    if not de_src:
        return _empty_entries(device, dtype)
    src_t = torch.tensor(de_src, dtype=torch.long, device=device)
    dst_t = torch.tensor(de_dst, dtype=torch.long, device=device)
    ships_t = torch.tensor(de_ships, dtype=dtype, device=device)
    ...
    return LaunchEntries(
        ...
        valid=torch.ones(len(de_src), dtype=torch.bool, device=device),
    )
...
def _apply_opportunist_boost(*, score, obs, target_idx, cand_tgt_short, config):
    """围魏救赵 / 趁火打劫 / 欲擒故纵: boost capturing ENEMY planets that are
    currently weak (low garrison) — a drained source that just launched at us,
    an over-extended planet, or one being eaten by a third party all read as low
    current ships. Lets the greedy take these targets of opportunity even when
    just under the ROI bar."""
    P = int(obs.P)
    pid = int(obs.player_id)
    tgt_abs = target_idx[cand_tgt_short].clamp(0, P - 1)
    tgt_owner = obs.owner_abs[tgt_abs]
    tgt_ships = obs.ships.to(score.dtype)[tgt_abs]
    weak_enemy = (tgt_owner >= 0) & (tgt_owner != float(pid)) & (tgt_ships <= float(config.opportunist_max_garrison))
    boost = float(config.opportunist_boost)
    return torch.where(weak_enemy.reshape(score.shape), score * boost, score)
...
def _plan_evacuation(*, movement, obs, cache, config):
    """Evacuate doomed planets. A planet that garrison_status projects to FALL
    (ships go negative from in-flight enemy fleets) ships its whole garrison to
    the nearest SAFE friendly planet reachable BEFORE the fall step -> the army
    survives and the attacker's overkill fleet lands on an empty planet, out of
    position. No prediction needed (reacts to already-launched fleets)."""
    ...
    if H <= 0:
        return _empty_entries(device, dtype)
    status = movement.garrison_status(max_horizon=H)
    falls = status.ships[:, 1:] < 0                              # [P, H] will-fall mask per step
    will_fall = owned & falls.any(dim=-1)
    doomed = will_fall & (obs.ships >= float(config.evac_min_ships))
    safe = owned & ~will_fall                                    # owned planets that hold
    if not bool(doomed.any()) or not bool(safe.any()):
        return _empty_entries(device, dtype)
    ...
    all_entries = []; launched = 0
    for s_i in range(int(src_idx.shape[0])):
        if launched >= int(config.evac_max_waves):
            break
        s = int(src_idx[s_i].item())
        gs = float(obs.ships[s].item())
        fs = int(fall_step[s].item())
        dists = d0[s, safe_idx]                                  # [Sf]
        speed = float(fleet_speed(torch.tensor(max(gs, 1.0), dtype=dtype, device=device)))
        etas = (dists / max(speed, 1e-6)).ceil()
        valid = (etas < float(fs)) & (safe_idx != s)            # arrive strictly before it falls
        if not bool(valid.any()):
            continue                                            # nowhere safe in time -> can't escape
        best = int(torch.where(valid, dists, torch.full_like(dists, 1e9)).argmin().item())
        dst = int(safe_idx[best].item())
        src_t = torch.tensor([[s]], dtype=torch.long, device=device)
        tgt_t = torch.tensor([dst], dtype=torch.long, device=device)
        send_t = torch.tensor([[gs]], dtype=dtype, device=device)   # whole garrison
        eta_t = torch.tensor([[float(etas[best].item())]], dtype=dtype, device=device)
        valid_t = torch.tensor([[True]], dtype=torch.bool, device=device)
        entry = make_launch_set(
            source_slots=src_t, target_slots=tgt_t.unsqueeze(-1).expand(1, 1),
            ships=send_t, eta=eta_t, valid=valid_t, player_id=pid,
        )
        all_entries.append(entry); launched += 1
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    if bool(getattr(config, "enable_evacuation", False)):
        evac_entries = _plan_evacuation(movement=movement, obs=obs, cache=cache, config=config)
...
CONFIG_4P = dataclasses.replace(
    ...
    enable_defense=True,   # 启用修复后的真防御 (owner-flip proactive reinforcement)
)
