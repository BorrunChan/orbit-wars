...
def _build_defense_entries(
    *,
    ...
    # code, ported from i-the-orbit). Use owner-flip. (fixed 2026-06-19)
    ...
    for t_i in range(int(tgt_indices.shape[0])):
        ...
        fs = int(fall_step[tgt].item())
        if abo is not None:                                # reinforcement-aware deficit
            inc = float(abo[tgt, 1:fs + 1, :].sum().item() - abo[tgt, 1:fs + 1, pid].sum().item())
        else:
            inc = float(ships_f[tgt].item()) + 1.0
        deficit = inc - float(ships_f[tgt].item())          # How many ships we need
        if deficit <= 0:
            continue
        need = deficit * float(config.defense_min_intercept_margin)

        # Find sources that have surplus ships and can reach BEFORE the fall
        ...
        send_ships = float(int(send_ships))   # Kaggle requires integer ship counts
        de_src.append(best_src); de_dst.append(tgt); de_ships.append(send_ships)

    if not de_src:
        return _empty_entries(device, dtype)
    # Build a proper LaunchEntries WITH angle — make_launch_set's LaunchSet (no
    # angle) crashes the downstream concat/infer.
