...
def _build_defense_entries(
    *,
    ...
    # ------------------------------------------------------------------
    # Step 1: find owned planets projected to FALL within the defense horizon.
    # garrison_status.ships is ABSOLUTE >=0, so the old `ships_at_H < 0` test
    # NEVER fired (dead code, ported from i-the-orbit). Use owner-flip like
    # _plan_evacuation does. (fixed 2026-06-19)
    ...
    if not bool(will_fall.any()):
        return _empty_entries(device, dtype)
    fall_step = falls.float().argmax(dim=-1) + 1                   # first fall step per planet [P]

    # ------------------------------------------------------------------
    # Step 2: for each threatened planet compute the reinforcement-aware enemy
    # incoming deficit (using arrivals_by_owner) and send the nearest in-time
    # surplus source to cover it. Build a proper LaunchEntries WITH angle —
    # make_launch_set's LaunchSet (no angle) crashes the downstream concat.
    d0 = cache.cross_dist[0].to(dtype)                            # [P, P]
    ships_f = obs.ships.to(dtype)
    abo = getattr(status, "arrivals_by_owner", None)
    tgt_indices = will_fall.nonzero(as_tuple=False).squeeze(1)
    ...
    for t in tgt_indices.tolist():
        ...
        deficit = inc - float(ships_f[t].item())                 # extra ships we still need
        if deficit <= 0:
            continue
        ...
        valid = (eta < float(fs)) & (surplus > 0)                # must arrive BEFORE the fall
