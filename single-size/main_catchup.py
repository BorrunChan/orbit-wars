...
class ProducerLiteConfig:
    ...
    # --- expansion value (fixes 4P under-attack) -------------------------
    # The competitive flow-diff only credits a capture's production over the
    # (horizon - eta) turns inside the projection window, so in 4P (short
    # horizon, higher roi) it scores clearable neutral expansions as a net ship
    # loss and the agent sits idle on planets it could take. This adds a tail-
    # value term ``neutral_expand_bonus * prod[target]`` to neutral-capture
    # candidates, representing production earned beyond the horizon window.
    neutral_expand_bonus: float = 0.0
    # Catch-up scaling of the expansion bonus: multiply by (1 + gain * deficit),
    # deficit = (leader_strength - my_strength)/my_strength clamped to [0, cap].
    # Trails leader -> expand harder (break the mid-game plateau); leads -> ~1.0.
    behind_expand_gain: float = 0.0
    behind_deficit_cap: float = 2.0
...
def plan_lite_waves(
    *,
    ...
    if int(player_count) >= 4 and (
        ...
        or float(config.neutral_expand_bonus) > 0.0
    ):
        ...
        # Expansion tail-value: reward capturing neutral planets for the
        # production they yield beyond the projection horizon.
        if float(config.neutral_expand_bonus) > 0.0:
            target_neutral = target_exists & obs.is_neutral[target_idx.clamp(0, P - 1)]
            # Catch-up scaling: replay analysis shows losses are a mid-game plateau
            # (we stall at ~4 planets 25-70% in while the winner climbs to 16). When
            # our economy trails the board leader, value expansion MORE to break the
            # plateau; when we lead, scale back to 1.0 (avoid over-extension that gets
            # us ganged -- the bootstrap/coop failure mode).
            leader_strength = strength.max().detach()
            deficit = ((leader_strength - my_strength) / my_strength.clamp(min=1.0)).clamp(
                0.0, float(config.behind_deficit_cap)
            )
            expand_scale = 1.0 + float(config.behind_expand_gain) * deficit
            neutral_bonus_short = torch.where(
                target_neutral,
                float(config.neutral_expand_bonus) * expand_scale
                * prod[target_idx.clamp(0, P - 1)].to(dtype),
                torch.zeros_like(target_bonus_short),
            )
            target_bonus_short = target_bonus_short + neutral_bonus_short
...
CONFIG_4P = dataclasses.replace(
    ...
    # Tunable during calibration via env NEB; bake the chosen value before ship.
    neutral_expand_bonus=3.0,
    behind_expand_gain=2.0,
)
