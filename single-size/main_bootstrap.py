...
def plan_lite_waves(
    *,
    ...
    # short horizon (you trade ~N ships now for only horizon-many turns of the
    # captured planet's production), so nothing clears roi and we stagnate to death.
    # When we own <=1 planet, have NO fleet in flight, and fired no wave this turn,
    # fire the single best candidate that already passed the viability + capture-floor
    # gates, ignoring roi (accept the short-term loss to escape the 1-planet trap).
    # Reuses the exact planner candidates; only the roi threshold is dropped.
