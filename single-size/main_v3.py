...
class ProducerLiteConfig:
    ...
    # --- multi-source coop -----------------------------------------------
    # When no single source can clear a target's capture floor, combine up to
    # ``max_coop_sources`` nearest sources into one candidate (contributor axis
    # L>1). The exact flow-diff scores the combined / sequential arrival, so a
    # coop wave only fires when it actually captures and beats roi.
    enable_coop: bool = True
    max_coop_sources: int = 3
    # --- regroup  ------------------------------
...
def plan_lite_waves(
    *,
    ...
    # --- pack candidates over a contributor axis L ------------------------------
    # Single-source candidates (one (source,target) each, active only in slot 0),
    # padded to L = max_coop_sources, plus one multi-source coop candidate per
    # target whose capture floor no single source clears.
    L = max(2, int(config.max_coop_sources)) if bool(config.enable_coop) else 1
    Cs = S * T

    ss_src = torch.zeros(Cs, L, dtype=torch.long, device=device)
    ss_src[:, 0] = source_idx.view(S, 1).expand(S, T).reshape(Cs)
    ss_send = torch.zeros(Cs, L, dtype=dtype, device=device)
    ss_send[:, 0] = torch.where(valid, sizes, torch.zeros_like(sizes)).reshape(Cs)
    ss_angle = torch.zeros(Cs, L, dtype=dtype, device=device)
    ss_angle[:, 0] = angle.reshape(Cs)
    ss_eta = torch.ones(Cs, L, dtype=dtype, device=device)
    ss_eta[:, 0] = torch.where(valid, eta, torch.ones_like(eta)).reshape(Cs)
    ss_active = torch.zeros(Cs, L, dtype=torch.bool, device=device)
    ss_active[:, 0] = valid.reshape(Cs)
    ss_tgt_slot = target_idx.view(1, T).expand(S, T).reshape(Cs)
    ss_tgt_short = torch.arange(T, device=device).view(1, T).expand(S, T).reshape(Cs)
    ss_valid = valid.reshape(Cs)

    co_rows = []
    if bool(config.enable_coop) and L >= 2 and S >= 2:
        v_l = viable.tolist(); se_l = source_exists.tolist(); te_l = target_exists.tolist()
        sizes_l = sizes.tolist(); floor_l = floor_at_arr.tolist()
        clears_l = clears_floor.tolist(); ang_l = angle.tolist(); eta_l = eta.tolist()
        src_ids = source_idx.tolist(); tgt_ids = target_idx.tolist()
        for t in range(T):
            if not te_l[t]:
                continue
            elig = [s for s in range(S)
                    if v_l[s][t] and se_l[s] and src_ids[s] != tgt_ids[t]]
            if len(elig) < 2:
                continue
            elig.sort(key=lambda s: sizes_l[s][t], reverse=True)
            if clears_l[elig[0]][t]:
                continue  # best single source already clears -> no coop needed
            pool = elig[:L]
            need = max(floor_l[s][t] for s in pool)
            chosen = []
            remaining = need
            for s in pool:
                snd = min(sizes_l[s][t], remaining)
                if snd <= 0.0:
                    break
                chosen.append((s, snd))
                remaining -= snd
                if remaining <= 0.0:
                    break
            if remaining > 1e-6 or len(chosen) < 2:
                continue  # cannot capture even combined (or single suffices)
            srcs = [0] * L; snds = [0.0] * L; angs = [0.0] * L; ets = [1.0] * L; acts = [False] * L
            for i, (s, snd) in enumerate(chosen):
                srcs[i] = src_ids[s]; snds[i] = snd
                angs[i] = ang_l[s][t]; ets[i] = eta_l[s][t]; acts[i] = True
            co_rows.append((srcs, snds, angs, ets, acts, tgt_ids[t], t))

    if co_rows:
        co_src = torch.tensor([r[0] for r in co_rows], dtype=torch.long, device=device)
        co_send = torch.tensor([r[1] for r in co_rows], dtype=dtype, device=device)
        co_angle = torch.tensor([r[2] for r in co_rows], dtype=dtype, device=device)
        co_eta = torch.tensor([r[3] for r in co_rows], dtype=dtype, device=device)
        co_active = torch.tensor([r[4] for r in co_rows], dtype=torch.bool, device=device)
        co_tgt_slot = torch.tensor([r[5] for r in co_rows], dtype=torch.long, device=device)
        co_tgt_short = torch.tensor([r[6] for r in co_rows], dtype=torch.long, device=device)
        co_valid = torch.ones(len(co_rows), dtype=torch.bool, device=device)
        cand_src = torch.cat([ss_src, co_src], dim=0)
        cand_send = torch.cat([ss_send, co_send], dim=0)
        cand_angle = torch.cat([ss_angle, co_angle], dim=0)
        cand_eta = torch.cat([ss_eta, co_eta], dim=0)
        cand_active = torch.cat([ss_active, co_active], dim=0)
        cand_tgt_slot = torch.cat([ss_tgt_slot, co_tgt_slot], dim=0)
        cand_tgt_short = torch.cat([ss_tgt_short, co_tgt_short], dim=0)
        cand_valid = torch.cat([ss_valid, co_valid], dim=0)
    else:
        cand_src, cand_send, cand_angle, cand_eta, cand_active = (
            ss_src, ss_send, ss_angle, ss_eta, ss_active)
    ...
    # --- starved bootstrap: guarantee expansion when the flow-diff planner stalls ---
    # On maps where our lone start is ringed by big (static-garrison) neutrals, the
    # competitive flow-diff scores every solo capture as a net ship LOSS inside the
    # short horizon, so nothing clears roi and we stagnate to death (coop can't help
    # here: with one planet there is no second source to combine). When we own <=1
    # planet, have NO fleet in flight, and fired no wave this turn, fire the single
    # best candidate that already passed the viability + capture-floor gates,
    # ignoring roi. Reuses the exact planner candidates; only the roi gate is dropped.
    if not bool(wave_entries.valid.any()) and int(obs.owned.sum().item()) <= 1:
        fleets_raw = obs_tensors.get("fleets")
        my_inflight = 0
        if fleets_raw is not None:
            my_inflight = int(
                ((fleets_raw[..., 0] >= 0) & (fleets_raw[..., 1] == float(pid))).sum().item()
            )
        if my_inflight == 0:
            boot_score = torch.where(
                cand_valid & ~cand_is_def, score, torch.full_like(score, float("-inf"))
            )
            if bool(torch.isfinite(boot_score).any()):
                b = _stable_argmax(boot_score)
                bsrc = cand_src[b]                                    # [L]
                btgt = cand_tgt_slot[b].view(1).expand(int(bsrc.shape[0]))
                wave_entries = LaunchEntries(
                    source_slots=bsrc.to(torch.long),
                    target_slots=btgt.to(torch.long),
                    ships=torch.where(
                        cand_active[b], cand_send[b], torch.zeros_like(cand_send[b])
                    ).to(dtype),
                    angle=cand_angle[b].to(dtype),
                    eta=cand_eta[b].to(dtype),
                    valid=cand_active[b],
                )
...
CONFIG_4P = dataclasses.replace(
    ...
    # Coop OFF in 4P: combining sources empties multiple planets at once, and in
    # FFA the emptied planets get captured by a third party (the same R4 failure
    # mode that forced v133 to restrict concentrated strikes to 2P). Replay
    # evidence (ep79251397) showed coop double-launches preceding 4P collapse.
    # Bootstrap stays global (it only fires when owned<=1, i.e. no planet to empty).
    enable_coop=False,
)
