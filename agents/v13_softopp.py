...
def _opp_starter_moves(state, opp_player):
    """Calibrated opp model: each opp planet with >=8 ships sends ~half toward
    its closest non-self planet (static or rotating). Mimics how real ladder
    bots open the game — they launch early with small fleets, not waiting for
    40+ ships."""
    moves = []
    for p in state["planets"]:
        if p[1] != opp_player or p[5] < 8:
            continue
        x0, y0 = p[2], p[3]
