    ...
    if _SHOT_TREES is not None:
        ...
        for c in candidates:
            ...
            if p_success >= shot_threshold:
                # v81: weight score by p_success so risky shots rank lower
                # even after passing threshold. Keeps deterministic shots up.
                weighted = (c[0] * p_success, c[1], c[2], c[3], c[4], c[5])
