"""
Orbit Wars - Agent v130 = TWO-LAYER architecture (attack + LOGISTICS).

Genuine architectural change, not a v124 feature tweak. Data-driven meta-
modeling of 998 real Kaggle replays (docs/Kaggle赛场对手与主流打法_数据建模.md)
+ reverse-engineering the top-13 players' (lb 1396-1636) decision policy found:

  1. Winners spend ~32% of launches on LOGISTICS — moving ships BETWEEN their
     own planets (rear -> frontier). v124 has ZERO logistics: every launch is
     an attack, so rear-planet ships hoard idle (Borrun: launch 0.45/turn,
     fleet 41 sitting on safe rear planets). The whole v98->v124 lineage caps
     at ~800 because it's attack-only.
  2. Winners sustain neutral expansion all game (30% of late-game targets are
     neutral vs our 11%). v124's candidate scoring DOUBLES enemy-target value,
     biasing it to abandon economic expansion and over-commit to costly fights.

v130 keeps v124's proven attack layer (our fleet size + expansion already
match the top per the data — that part isn't broken) and adds:
  - LOGISTICS LAYER: shuttle surplus from rear (safe, no nearby enemy) planets
    to the nearest frontier planet, so ships reach the front instead of rotting.
  - VALUATION FIX: reduce the enemy-capture bonus (2x -> 1.3x) to sustain
    neutral expansion.

Validation honesty: local bench is anti-correlated with Kaggle (proven this
session: v98->v124 local "gains" = Kaggle losses). v130 is a Kaggle-validated
bet grounded in winner behavior, NOT tuned to local bench.
"""
...
# === v123: SWARMER DETECTION via cumulative opp launch rate ===
# Track cumulative opp launches via next_fleet_id delta vs our own count.
# Real Kaggle replay analysis (140 games):
#   41/45 of our 2P losses had opp launches > 3x mine. Median opp_launch_rate
#   in losses = 3-8 launches/turn. v98 doesn't react to this signal.
# Trigger:  num_players==2 AND step >= 25 AND opp_launch_rate >= 3.0/turn
# Action:   unlock max_launch floor (ships-2) + lower shot_threshold (-0.10)
# Reset:    on step <= 1 (new game)
_V123_INIT_NFID = None       # next_fleet_id snapshot at game start
_V123_MY_LAUNCH_TOTAL = 0    # cumulative my launches this game
...
def agent(obs):
    ...
    next_fleet_id_now = getf("next_fleet_id", 0) or 0

    # v123: detect new game on step <= 1, snapshot starting next_fleet_id
    global _V123_INIT_NFID, _V123_MY_LAUNCH_TOTAL
    if step_now <= 1 or _V123_INIT_NFID is None:
        _V123_INIT_NFID = next_fleet_id_now
        _V123_MY_LAUNCH_TOTAL = 0
    ...
    # v124: 2P EARLY-GAME max_launch floor.
    # 56-game v123 Kaggle data: 22/34 losses (65%) had fell_behind in step
    # 11-30. Opps launched 3-25 fleets in first 20 steps vs our 1-7. v98's
    # _max_dispatch defensively returns low values when threats accumulate,
    # paralyzing our expansion. Unlock 25% ship floor so we keep firing.
    # v123 late trigger (step>=25, rate>=3.0) covers confirmed swarm case;
    # v124 covers early window 10-60 step (peak-then-collapse failure mode).
    # Reuses v92's proven floor mechanism (already +pp in 4P).
    early_swarm_2p = (num_players == 2 and 10 <= step_now < 60
                       and _n_total >= 3)
    ...
    for mine in my_planets:
        ...
        if extreme_swarm:
            ...
            # v124: 25% floor for 2P early-game swarm pressure
            forced_min = int(mine.ships * 0.25)
            max_launch[mine.id] = max(safe, forced_min)
        else:
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            if tgt.owner != -1:
                # v130: enemy bonus reduced 2x -> 1.3x. Winners stay 30%
                # neutral late; v124's full double-value abandoned neutral
                # expansion (we hit 11% late). Keep some swing credit, not 2x.
                base_value += 0.3 * tgt.production * time_owned
    ...
    # === v123: CUMULATIVE-LAUNCH-RATE SWARMER DETECTION ===
    # Orthogonal signal to v95's in-flight-fleet count. Real Kaggle replays
    # show 41/45 2P losses had opp accumulating >>3 launches/turn over the
    # game. next_fleet_id is a global counter, so:
    #   opp_total = (current_nfid - initial_nfid) - my_total_launches
    # Trigger only in 2P after step 25 to give v98 normal time to expand
    # against structured opponents; if vs structured (low launch rate), this
    # NEVER triggers — agent() stays byte-identical to v98.
    v123_swarmer_confirmed = False
    if num_players == 2 and step_now >= 25:
        opp_total_launches = max(0,
            (next_fleet_id_now - _V123_INIT_NFID) - _V123_MY_LAUNCH_TOTAL)
        opp_launch_rate = opp_total_launches / max(1, step_now)
        if opp_launch_rate >= 3.0:
            v123_swarmer_confirmed = True
            swarm_active = True  # also raise existing v95 flag
            # Aggressive shot threshold drop on top of v95's adjustments
            shot_threshold = max(0.08, shot_threshold - 0.10)
            # Unlock max_launch floor to ships - 2 (override _max_dispatch)
            for _mp in my_planets:
                _floor = max(0, _mp.ships - 2)
                if max_launch.get(_mp.id, 0) < _floor:
                    max_launch[_mp.id] = _floor
    ...
    # === v130: LOGISTICS LAYER ===
    # The architectural addition. After the attack layer allocates captures,
    # shuttle leftover surplus from REAR planets (safe, no nearby enemy, no
    # incoming threat) toward the nearest FRONTIER planet. This stops rear
    # ships from rotting idle — the core thing the top-13 winners do (~32% of
    # their launches) that v124 never did.
    # Gate: only consolidate when we have breathing room. Under swarm pressure
    # (v95/v123 swarm_active), shuttling strips planets mid-move and they fall —
    # confirmed drain vs proto1000 (0/4, eliminated). Winners consolidate when
    # safe and defend when pressed; logistics is a peacetime activity.
    logistics_moves = []
    enemy_ps = [p for p in planets if p.owner != player and p.owner != -1]
    if enemy_ps and len(my_planets) >= 2 and not swarm_active:
        FRONTIER_DIST = 35.0   # within this of any enemy => frontier
        MIN_SHUTTLE = 8        # don't bother moving tiny amounts

        def _nearest_enemy_dist(mp):
            return min(math.hypot(mp.x - e.x, mp.y - e.y) for e in enemy_ps)

        frontier = [mp for mp in my_planets
                    if threats_per_planet[mp.id]
                    or _nearest_enemy_dist(mp) <= FRONTIER_DIST]
        rear = [mp for mp in my_planets if mp not in frontier]

        if frontier and rear:
            for src in rear:
                # shuttle 60% of surplus, keep 40% as rear reserve (don't strip
                # rear planets bare — they may become frontier later)
                surplus = int((max_launch[src.id] - used[src.id]) * 0.6)
                if surplus < MIN_SHUTTLE:
                    continue
                # destination: frontier planet under threat first, else nearest
                threatened = [f for f in frontier if threats_per_planet[f.id]]
                pool = threatened if threatened else frontier
                dst = min(pool, key=lambda f: math.hypot(src.x - f.x,
                                                          src.y - f.y))
                # rendezvous with our own (possibly rotating) planet: iterate
                # a couple times to aim where it WILL be on arrival.
                aim = (dst.x, dst.y)
                ang = None
                for _ in range(3):
                    a = _safe_launch_angle((src.x, src.y), aim, dst.radius)
                    if a is None:
                        break
                    dist = math.hypot(aim[0] - src.x, aim[1] - src.y)
                    T = max(1, math.ceil(dist / _speed(surplus)))
                    pos = _predict_position(dst.id, (dst.x, dst.y),
                                            initial_by_id, angular_velocity,
                                            step_now, step_now + T, comet_groups)
                    if pos is None:
                        break
                    if math.hypot(pos[0] - aim[0], pos[1] - aim[1]) < 0.5:
                        ang = a
                        aim = pos
                        break
                    aim = pos
                ...
                if ang is None:
                    continue
                logistics_moves.append([src.id, ang, int(surplus)])
                used[src.id] += surplus

    # v123: update cumulative launch counter for next-step opp rate estimate
    _V123_MY_LAUNCH_TOTAL += len(accepted) + len(logistics_moves)

    # v130: attacks first, then logistics shuttles
    return accepted + logistics_moves
