...
def agent(obs):
    ...
    # v117: BYPASS-SIM PRESSURE SHOTS (only 2P).
    # Diagnostic: v98 launches 142/game vs proto1000's 1966 (13.9x). v98's
    # candidate gen only emits "full capture" candidates (skips when
    # _capture_cost returns None for high-garrison targets). proto1000 fires
    # 5 ships regardless. Add post-greedy small fixed-size launches at nearest
    # enemy planet, without sim validation. Cap at 5 ships per mine per turn.
    if num_players == 2:
        PRESSURE_SHIPS = 5
        for mine in my_planets:
            remaining = max_launch.get(mine.id, 0) - used.get(mine.id, 0)
            if remaining < PRESSURE_SHIPS:
                continue
            enemy_tgts = [t for t in targets if t.owner != -1]
            neut_tgts = [t for t in targets if t.owner == -1]
            avail_tgts = enemy_tgts if enemy_tgts else neut_tgts
            if not avail_tgts:
                continue
            tgt_p = min(avail_tgts,
                        key=lambda t: math.hypot(t.x - mine.x, t.y - mine.y))
            angle_p = _safe_launch_angle((mine.x, mine.y),
                                          (tgt_p.x, tgt_p.y),
                                          tgt_p.radius)
            if angle_p is None:
                continue
            accepted.append([mine.id, angle_p, PRESSURE_SHIPS])
            used[mine.id] = used.get(mine.id, 0) + PRESSURE_SHIPS
