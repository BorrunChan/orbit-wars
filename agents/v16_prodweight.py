...
def _simulate(state, my_actions, opp_actions, horizon, player, num_players=2,
    ...
    for _ in range(horizon - 1):
        ...
        sim.step(state, actions)
    return _eval_v16(state, player)


def _eval_v16(state, player):
    """Eval with heavy future-production weighting + planet-count term."""
    my_ships = opp_ships = 0
    my_prod = opp_prod = 0
    my_pl = opp_pl = 0
    for p in state["planets"]:
        if p[1] == player:
            my_ships += p[5]; my_prod += p[6]; my_pl += 1
        elif p[1] != -1:
            opp_ships += p[5]; opp_prod += p[6]; opp_pl += 1
    ...
    return (my_ships - opp_ships) \
           + (my_prod - opp_prod) * remaining * 1.5 \
           + (my_pl - opp_pl) * 20
