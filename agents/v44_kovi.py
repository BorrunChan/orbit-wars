...
# v44: tuned toward kovi/typeIIIfairy profile observed in Kaggle replays.
# Top players have: avg_ships ~22 (we had 9), p90 ~37 (we had 14),
# launch_rate ~0.25 (we had 0.11). Approach: lower shot threshold (more
# launches survive filter) + prefer buffered fleet variant (bigger ships) +
# raise candidate cap K (more origins participate).
SHOT_REJECT_THRESHOLD = 0.30   # was 0.40
SHOT_THRESHOLD_SLOW = 0.12     # was 0.20

SHOT_THRESHOLD_4P_SLOW = 0.20  # was 0.30
SHOT_THRESHOLD_4P_FAST = 0.35  # was 0.50
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            if tgt.owner != -1:
                base_value += tgt.production * time_owned
            # v44: center push — top players drift centrality_drift = -8 (move
            # inward); we drift -1.7 (stay put). Weight central targets higher.
            tgt_dist_to_center = math.hypot(tgt.x - CENTER, tgt.y - CENTER)
            center_bonus = 1.0 + 0.6 / (tgt_dist_to_center + 5.0)
            ...
            # Variant 2: buffered capture — v44 doubles buffer (kovi sends
            # avg 24, we sent 9). Now buffer = min(min_ships, avail - min_ships)
            # so the bigger fleet is ~2× the minimum when ships available.
            buf = min(min_ships + 10, avail - min_ships)
            if buf > 0:
                big_ships = min_ships + buf
                # Slightly less discount than before (was 0.95). Prefer bigger.
                candidates.append((base_value / (big_ships + 1.0 * T) * 0.98,
    ...
    # v44: raise K from 15 to 40 — top players use 74% of planets as origins
    # (we used 32%). Larger K lets more origins reach the sim validation step.
    K = min(40, len(candidates))
    grouped = {}
