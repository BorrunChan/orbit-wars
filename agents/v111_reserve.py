...
def agent(obs):
    ...
    # v111: B5 RESERVE — keep core garrison against medium swarmer.
    # 27-replay analysis: medium swarmer (200-500 LR) is 2P main pain point
    # (5L/9 = 44% loss). Reserving 8 ships per planet creates a buffer
    # against the 1-2 incoming fleets per turn. Only 2P.
    RESERVE_PER_PLANET = 8 if num_players == 2 else 0
    ...
    for mine in my_planets:
        ...
        if extreme_swarm:
            ...
            # v111: subtract reserve to keep garrison
            max_launch[mine.id] = max(0, safe - RESERVE_PER_PLANET)
