...
def agent(obs):
    ...
    max_launch = {}
    # v119: in 2P, step < 20 = expansion race window. No enemy fleets can reach
    # us (fleet speed ~1.5/turn × 20 turns = 30 units < 50 units to home).
    # v98 _max_dispatch was too conservative pre-step-20, letting proto1000
    # free-expand to 9 planets by step 30 while we sat at 5. Force aggressive
    # floor (ships - 5) so capture candidates are available.
    early_race = (num_players == 2 and step_now < 20)
    for mine in my_planets:
        ...
        if extreme_swarm:
            ...
            # Reserve only 5 ships for defense; rest available for expansion
            max_launch[mine.id] = max(safe, max(0, mine.ships - 5))
        else:
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            if tgt.owner != -1:
                base_value += tgt.production * time_owned
            # v119: early-race neutral boost — race proto1000-class opponents
            # for first ~20 planets. 2x value makes sim greedy preferentially
            # accept these expansions over "no-action" alternatives.
            if early_race and tgt.owner == -1:
                base_value *= 2.0
