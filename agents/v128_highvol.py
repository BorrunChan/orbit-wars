"""
v128 — HIGH-VOLUME paradigm agent (deployable derivative of proto1000).

PARADIGM PIVOT, not a v124 tweak. v124's precision-defense line goes 0/23
vs real Kaggle high-volume swarmers and caps at <850. This agent IS the
high-volume meta: send exactly-enough-to-capture from many planets, track
en-route to avoid waste, reinforce planets under attack, coop when a single
planet is insufficient. "Release all havoc" (ignore en-route cap) when we
own < 75% of planets.

Provenance: structurally derived from the public proto1000 notebook bot
(in opponents/proto1000.py). numpy import removed (was unused) so the file
is stdlib + the runtime-available orbit_wars module only, meeting the
submission constraint. Validated locally: beats v124 75% (2P) / 100% (4P),
holds standard pool at 62% (v124 70%), max decision time 10ms (<1000ms).
"""
import math
