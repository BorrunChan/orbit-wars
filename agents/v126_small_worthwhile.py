"""
Orbit Wars - Agent v126 (small-worthwhile probe layer on top of v124).

Hypothesis: real Kaggle data (196 games) shows our 2P WR is monotonic
in opp submit_rate — we win 70% at <1.5/turn but 0% at >=4.0/turn. v124's
sim greedy rejects 84-94% of candidates (verified via instrumentation),
which caps our launches at ~0.46/turn. Against high-volume opps, this is
the structural blind spot.

This agent adds an ADDITIVE second pass: after v124's sim greedy runs,
when v95 swarm_active fires (n_small >= 3 or expansion_deficit, 2P only),
take small cheap candidates that already passed shot_model but failed sim,
and fire them without sim validation. Bounded by:
  - ships <= PROBE_SHIP_CAP (12)
  - eta T <= PROBE_T_CAP (12)
  - max PROBE_MAX (5) probes per turn
  - respects max_launch[mid] (v124's defense calc stays sacred)

Differs from past failures:
  - v108/v110: replaced filter, no defense floor → drain to 0%
  - v117: 5 ships/mine bypass-sim, no max_launch respect → drain
  - v125: relaxed targeted lock inside sim greedy → still got sim-rejected
  - v126: bypasses sim ONLY when swarm detected, respects max_launch ceiling
"""
...
def agent(obs):
    ...
    # === v126: SMALL-WORTHWHILE PROBE PASS ===
    # Run only in 2P when v95 swarm_active fires. Takes top-scored small
    # candidates (already passed shot_model) that failed sim greedy and
    # fires them without sim validation. Bounded so it can never drain.
    if num_players == 2 and swarm_active:
        PROBE_SHIP_CAP = 12
        PROBE_T_CAP = 12
        PROBE_MAX = 5
        probe_pool = []
        for c in candidates:
            ...
            if max_launch[mid] - used[mid] < ships:
                continue
            probe_pool.append(c)
        probe_pool.sort(reverse=True, key=lambda c: c[0])
        probes_fired = 0
        for c in probe_pool:
            if probes_fired >= PROBE_MAX:
                break
            score, mid, tid, ships, angle, T = c
            if tid in targeted:
                continue
