...
def _ensemble_eval(state_base, my_actions, horizon, player, num_players):
    """Run two sims with different opp assumptions (starter and idle), average
    the evals. More robust to unknown opp behavior."""
    s1 = sim.clone(state_base)
    e1 = _simulate(s1, my_actions, [], horizon, player, num_players, "starter")
    s2 = sim.clone(state_base)
    e2 = _simulate(s2, my_actions, [], horizon, player, num_players, "none")
    return (e1 + e2) * 0.5
...
def agent(obs):
    ...
    baseline_eval = _ensemble_eval(sim_state_base, [], SIM_HORIZON, player,
                                     num_players)
    ...
    for key in group_keys:
        ...
        for score, _mid, _tid, ships, angle, T in grouped[key]:
            ...
            new_eval = _ensemble_eval(sim_state_base, trial_moves, SIM_HORIZON,
                                        player, num_players)
