...
# v115: cross-turn state for opp behavior detection.
# Tracks max opp fleets in flight (proxy for launch frequency).
_OPP_STATE = {
    "max_opp_fleets": 0,
    "last_step": -1,
    "high_volume_locked": False,  # sticky once detected
}
...
def _opp_high_volume_moves(state, opp_player):
    """v115: model for high-volume opp like proto1000 (MIN_SHIPS=5).
    Every planet with >=5 ships fires at closest non-self target.
    Used by sim when v98 detects opp is high-volume."""
    moves = []
    for p in state["planets"]:
        if p[1] != opp_player or p[5] < 5:
            continue
        x0, y0 = p[2], p[3]
        best = None
        best_d = float('inf')
...
def _simulate(state, my_actions, opp_actions, horizon, player, num_players=2,
    ...
    what we expect the opponent to do each future turn:
      ...
      - "high_volume": every planet >=5 ships fires (v115, proto1000-like)
    ...
    for _ in range(horizon - 1):
        ...
        elif opp_model == "high_volume":
            for opp in range(num_players):
                if opp == player:
                    continue
                opp_moves = _opp_high_volume_moves(state, opp)
                if opp_moves:
                    actions[opp] = opp_moves
        sim.step(state, actions)
...
def agent(obs):
    ...
    # v115: detect high-volume opp behavior in 2P. Track max opp fleets in
    # flight across turns. Reset on new game (step regress).
    if step_now < _OPP_STATE["last_step"] or step_now <= 1:
        _OPP_STATE["max_opp_fleets"] = 0
        _OPP_STATE["high_volume_locked"] = False
    _OPP_STATE["last_step"] = step_now
    cur_opp_fleets = sum(1 for f in fleets if f.owner != player and f.owner >= 0)
    if cur_opp_fleets > _OPP_STATE["max_opp_fleets"]:
        _OPP_STATE["max_opp_fleets"] = cur_opp_fleets
    # Lock high_volume sticky once threshold breached (after a few turns)
    if num_players == 2 and step_now >= 5 and \
            _OPP_STATE["max_opp_fleets"] >= 3:
        _OPP_STATE["high_volume_locked"] = True
    ...
    # v115: in 2P, if opp is high-volume (>=3 fleets in flight by step 5+),
    # switch to "high_volume" model so sim correctly predicts pressure.
    # v116: unconditionally use high_volume in 2P (no detection gating)
    if num_players == 4:
        sim_opp_model = "none"
    else:
        sim_opp_model = "high_volume"
