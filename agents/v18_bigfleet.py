"""
Orbit Wars - Agent v18

Pure heuristic, no sim filter. Key idea: each attack sends max(min_capture,
mine.ships // 2). Bigger fleets so captured planets aren't fragile.

Lessons from earlier iterations:
  - v2d (min capture) loses 0% vs strong opps because min captures leave
    1-ship planets that get retaken
  - v3p (sim filter) hoards because sim's "do nothing" baseline looks safe
  - Maybe a starter-style "always send half ships at best target" with my
    full target evaluation will be more robust

This is intentionally simple and AGGRESSIVE — no sim, no filtering.
"""
...
def agent(obs):
    ...
    # Threats → max launchable per mine
    ...
    for mine in my_planets:
        max_launch[mine.id] = _max_dispatch(mine, sorted(threats_per_planet[mine.id]))

    # Build candidate list: best target for each mine, with BIG fleet.
    candidates = []
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            # BIG fleet: max(min_capture, half_source). Bigger fleets are
            # faster AND leave more ships on the planet after capture.
            big_ships = max(min_ships, avail // 2)
            big_ships = min(big_ships, avail)
            # Recompute T with bigger fleet
            dist = math.hypot(arrival[0] - mine.x, arrival[1] - mine.y)
            fleet_T = max(1, math.ceil(dist / _speed(big_ships)))
            angle = _safe_launch_angle((mine.x, mine.y), arrival, tgt.radius)
            if angle is None:
                continue
            time_owned = max(0, game_remaining - fleet_T)
            value = tgt.production * time_owned
            if tgt.owner != -1:
                value += tgt.production * time_owned
            score = value / (big_ships + 0.5 * fleet_T)
            candidates.append((score, mine.id, tgt.id, big_ships, angle, fleet_T))
    ...
    for score, mid, tid, ships, angle, T in candidates:
        ...
        if budget < ships:
            # Try sending what we can (lower bound: min capture would still work)
            continue
        used[mid] += ships
        targeted.add(tid)
