...
def agent(obs):
    ...
    # 2P swarm trigger: ≥6 small fleets
    # 4P swarm trigger: ≥15 (high bar — proto1000-style spam in local pool
    # creates ~10 fleets routinely; need higher threshold to avoid false trip)
    swarm_threshold = 15 if num_players == 4 else 6
    if n_small >= swarm_threshold:
        swarm_active = True
        # 2P: soften threshold AND accept low-cost shots (v84 logic).
        # 4P: more conservative — just allow the low-cost shot path.
        if num_players == 2:
    ...
    for key in group_keys:
        ...
        for score, _mid, _tid, ships, angle, T in grouped[key]:
            ...
            # v87: sim tolerance — accept if not worse by more than `tol`
            tol = 3.0 if swarm_active else 0.0
            if new_eval > best_eval_for_group - tol:
                # If improving, advance; if slight loss but tolerated, still accept
                best_eval_for_group = max(new_eval, best_eval_for_group)
