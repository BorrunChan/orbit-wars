...
def agent(obs):
    ...
    # --- Phase 2: sim subtractive prune (with aggressive opp model) ---
    sim_state_base = sim.make_state_from_obs(obs)
    trial_state = sim.clone(sim_state_base)
    current_eval = _simulate(trial_state, heuristic, SIM_HORIZON, player,
                              num_players, opp="aggressive")

    i = 0
    while i < len(heuristic):
        trial = heuristic[:i] + heuristic[i+1:]
        trial_state = sim.clone(sim_state_base)
