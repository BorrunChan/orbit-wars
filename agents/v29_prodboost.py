...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            # Winner traces show median target prod=3 for top bots, while our
            # v20 chooses prod=1 targets. Boost high-prod targets in scoring.
            prod_boost = 1.0 + 0.5 * max(0, tgt.production - 2)
    ...
    # === Early-launch override (from winner-trace analysis) ===
    # lb1224 / structured launch as early as turn 1; v20 typically waits to
    # turn 10+. In early game, sim's "do nothing" baseline is misleading
    # (opp model is too passive). Force the top heuristic candidate.
    if step_now < 6 and len(accepted) == 0 and candidates:
        used_force = {p.id: 0 for p in my_planets}
        for cand in candidates[:6]:
            score, mid, tid, ships, angle, T = cand
