"""
Orbit Wars - Agent v129 = v124 + reactive defensive reinforcement.

Motivated by data-driven meta-modeling (docs/Kaggle赛场对手与主流打法_数据建模.md,
998 real replays): high-volume is NOT the meta; we already match the top on
fleet size + expansion; the 776→1636 gap is execution depth. Defense is one
untested axis — v124 never reinforces its own planets (proto1000 does).

Adds: when one of our worthwhile planets (production>=2) is doomed by incoming
threats, dispatch a single fully-covering savior from the nearest planet that
can afford it and arrive in time, only if the saved production is worth the
ships. Refined from the (locally-failed, never-Kaggle-tested) v106; v106's
local-bench failure is uninformative because that bench is anti-correlated
with Kaggle. This is meant to be validated on Kaggle, not local bench.
"""
...
def agent(obs):
    ...
    # === v129: REACTIVE DEFENSIVE REINFORCEMENT (refined from v106) ===
    # v124 only USES _doom_deficit to self-quarantine (max_launch=0); it never
    # sends a savior. Real Kaggle meta-modeling (998 games) shows the rating
    # gap is execution depth, not style — and defense is one untested axis.
    # proto1000 reinforces; v124 doesn't. v106 tried this but (a) was only ever
    # judged on the anti-correlated local bench, (b) saved EVERY doomed planet
    # incl. worthless fringe ones, throwing good ships after bad.
    # v129 refinements vs v106:
    #   - only save planets worth saving: production >= 2
    #   - only if ONE source can fully cover the deficit (no half-saves)
    #   - skip if the saved planet's remaining-game value < ships committed
    #   - cap one reinforcement per doomed planet; reserve ships from attacker
    reinforcement_moves = []
    for mp_d in my_planets:
        if mp_d.production < 2:
            continue  # fringe low-value planet — let it fall
        threats_d = sorted(threats_per_planet[mp_d.id])
        deficit_info = _doom_deficit(mp_d, threats_d)
        if deficit_info is None:
            continue
        failing_turn, deficit_ships = deficit_info
        send_ships = int(deficit_ships * 1.2) + 3
        # value of saving = production held for the rest of the game
        save_value = mp_d.production * max(0, game_remaining - failing_turn)
        if save_value < send_ships:
            continue  # not worth the ships
        by_dist = sorted(my_planets,
                          key=lambda m: math.hypot(m.x - mp_d.x, m.y - mp_d.y))
        for src in by_dist:
            if src.id == mp_d.id or src.production < 1:
                continue
            dist = math.hypot(src.x - mp_d.x, src.y - mp_d.y)
            if dist < 1e-6:
                continue
            eta = max(1, math.ceil(dist / _speed(send_ships)))
            if eta >= failing_turn:
                continue  # too late
            if max_launch.get(src.id, 0) < send_ships:
                continue  # one source must fully cover (no half-save)
            pred = _predict_position(mp_d.id, (mp_d.x, mp_d.y), initial_by_id,
                                      angular_velocity, step_now,
                                      step_now + eta, comet_groups)
            if pred is None:
                continue
            angle = _safe_launch_angle((src.x, src.y), pred, mp_d.radius)
            if angle is None:
                continue
            reinforcement_moves.append([src.id, angle, int(send_ships)])
            max_launch[src.id] -= send_ships  # reserve before attack alloc
            break
    ...
    _V123_MY_LAUNCH_TOTAL += len(accepted) + len(reinforcement_moves)

    # v129: prepend reinforcement (defensive) moves before attacks
    return reinforcement_moves + accepted
