...
# v83: Kaggle 2P opps use SWARM (many small fleets). v69 too restrictive.
# Soften 2P thresholds + add low-cost shot acceptor.
SHOT_REJECT_THRESHOLD = 0.32   # was 0.40 — let smaller fleets through
SHOT_THRESHOLD_SLOW = 0.15     # was 0.20

SHOT_THRESHOLD_4P_SLOW = 0.25  # unchanged (4P at 53% on Kaggle, don't break)
SHOT_THRESHOLD_4P_FAST = 0.45

# v83: low-cost acceptor — even moderate p_success is OK if commitment is small
LOW_COST_SHIPS_MAX = 12   # send ≤12 ships
LOW_COST_T_MAX = 10        # arrive within 10 ticks
LOW_COST_P_MIN = 0.18      # P(success) ≥ 0.18 is fine for cheap shots
...
def agent(obs):
    ...
    if _SHOT_TREES is not None:
        ...
        for c in candidates:
            ...
            # v83: dual-track filter — high-conviction OR low-cost
            if p_success >= shot_threshold:
                filtered.append(c)
            elif (num_players == 2  # only 2P (4P has different dynamics)
                  and ships <= LOW_COST_SHIPS_MAX
                  and eta <= LOW_COST_T_MAX
                  and p_success >= LOW_COST_P_MIN):
                filtered.append(c)
        candidates = filtered
