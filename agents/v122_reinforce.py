...
def agent(obs):
    ...
    # v122: PROACTIVE REINFORCEMENT (proto1000-style).
    # v98 already has _doom_deficit to detect planets that will fall. It's used
    # only via _max_dispatch to LIMIT attacks. proto1000 also actively dispatches
    # reinforcements from neighbors when a planet is doomed — v98 has no such
    # mechanism. Coop attack (v121) failed because we still lost home planets
    # to coop-attacking proto1000 between our attacks. Defense-first this time.
    reinforcement_moves = []
    if num_players == 2:
        for mp in my_planets:
            thr = sorted(threats_per_planet[mp.id])
            doom = _doom_deficit(mp, thr)
            if doom is None:
                continue
            failing_turn, deficit = doom
            spare = []
            for src in my_planets:
                if src.id == mp.id:
                    continue
                src_rem = max_launch.get(src.id, 0)
                if src_rem < 5:
                    continue
                dist = math.hypot(src.x - mp.x, src.y - mp.y)
                eta = dist / _speed(5)
                if eta > failing_turn:
                    continue
                spare.append((src, src_rem, dist))
            spare.sort(key=lambda x: x[2])
            sent = 0
            for src, src_rem, dist in spare:
                if sent >= deficit:
                    break
                send = min(src_rem, max(5, deficit - sent))
                angle = math.atan2(mp.y - src.y, mp.x - src.x)
                reinforcement_moves.append([src.id, angle, int(send)])
                max_launch[src.id] -= send
                sent += send
    ...
    # v122: prepend reinforcement moves so sim greedy sees them
    accepted = list(reinforcement_moves)
