"""
v134 — v133 + garrison reserve on concentrated strikes.

v133 fixed 4P R4 (36%->10%) by disabling concentrated strike in 4P.
But 2P still bleeds: on losing seeds (123/3141/6060) v133 sends 78 ships
from a planet, it gets recaptured by structured's counterattack.

Fix: concentrated strike sends max 70% of garrison (keep 30% reserve).
Applies to both 2P and 4P. This matches 900-1000 opponent behavior
(data: avg_garrison never drops to 0 in their wins OR losses).
"""
import math
        ...
        for m, t, s in candidate_targets[:3]:
            ...
            if m_available_ships >= base_ships: # single attack
                ...
                if m.id not in under_attack and m_available_ships >= 40:
                    send_ships = int(m_available_ships * 0.7)
