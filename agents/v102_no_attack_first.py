...
def _estimate_rank(planets, fleets, player, num_players):
    """Composite score per player, returns (my_rank_1based, gap_above_norm,
    gap_below_norm). gap_above = (score of next-higher rival - me) / total;
    gap_below = (me - score of next-lower rival) / total. Used to detect a
    'stable 2nd' state in 4P where risking ships to catch #1 is worse EV
    than locking the place."""
    scores = [0.0] * num_players
    for p in planets:
        if 0 <= p.owner < num_players:
            scores[p.owner] += p.ships + p.production * 8.0
    for f in fleets:
        if 0 <= f.owner < num_players:
            scores[f.owner] += f.ships * 0.7
    my_score = scores[player]
    rank = 1 + sum(1 for i, s in enumerate(scores) if i != player and s > my_score)
    sorted_desc = sorted(scores, reverse=True)
    if rank > 1:
        gap_above = sorted_desc[rank - 2] - my_score
    else:
        gap_above = float('inf')
    if rank < num_players:
        gap_below = my_score - sorted_desc[rank]
    else:
        gap_below = float('inf')
    total = max(1.0, sum(scores))
    # v102: also return rank-1 player id (-1 if it's us, i.e. rank == 1)
    top_score = sorted_desc[0]
    leader = -1
    if rank > 1:
        for i, s in enumerate(scores):
            if i != player and s == top_score:
                leader = i
                break
    return rank, gap_above / total, gap_below / total, leader
...
def agent(obs):
    ...
    # v101: rank-aware "stable 2nd" detection (4P only).
    # When we're rank 2 with comfortable gap below (#3 far behind) AND #1 is
    # far enough ahead to be unrealistic to catch, switch to consolidation:
    #   - skip neutral-expansion candidates (don't bleed ships on new fronts)
    #   - tighten shot_threshold (only high-confidence shots fire)
    # Activates only mid-late game (step >= 40) to let early expansion play out.
    stable_second = False
    leader_id = -1
    if num_players == 4 and step_now >= 40:
        rank, gap_above, gap_below, leader_id = _estimate_rank(
            planets, fleets, player, num_players)
        if rank == 2 and gap_below >= 0.08 and gap_above >= 0.06:
            stable_second = True
    ...
    for mine in my_planets:
        ...
        if avail <= 1:
            continue
        for tgt in targets:
            # v101: in stable-2nd mode, skip neutral expansion (saves ships for
            # defense + reinforcement; can't catch #1 anyway, must not drop to #3).
            # v102: also skip attacks on the rank-1 player — attacking #1 is
            # waste of ships when we can't realistically catch them. Only attack
            # rank-3/4 if it consolidates our position.
            if stable_second and tgt.owner == -1:
                continue
            if stable_second and leader_id >= 0 and tgt.owner == leader_id:
                continue
    ...
    # v101: stable-2nd tightening — only fire high-confidence shots.
    if stable_second:
        shot_threshold = min(0.65, shot_threshold + 0.08)
