"""
v132 — v131 + situational awareness layer.

v131 base (proto1000 derivative with MIN_SHIPS=8, neutral bonus, concentrated
strikes). v131's fatal flaw: identical behavior whether winning or losing.
Replay analysis of 78 our losses + 92 losses by 900-1000 opponents + 15 losses
by 1000+ opponents reveals:

  When 900-1000 players LOSE, avg_garrison goes 35 -> 98 (they accumulate).
  When WE lose, avg_garrison goes 39 -> 28 (we bleed out via small fleets).

v132 adds a situation assessment layer that changes behavior based on power:
  DOMINANT (>55% planets): full aggression (v131 logic)
  EVEN (40-55%): balanced, only feasible targets
  BEHIND (25-40%): accumulate garrison to 40+, only cheap targets
  DESPERATE (<25%): concentrate on 1-2 planets, accumulate to 80+, all-in

Also: ETA filter (don't launch fleets arriving after step 500), feasibility
filter (don't attack targets we can't take), early-game neutral priority.
"""
import math
...
def assess_situation(mine, targets, step):
    my_count = len(mine)
    my_prod = sum(p.production for p in mine)
    my_ships = sum(p.ships for p in mine)
    enemy_targets = [t for t in targets if t.owner != -1]
    enemy_count = len(enemy_targets)
    enemy_prod = sum(t.production for t in enemy_targets)
    enemy_ships = sum(t.ships for t in enemy_targets)
    total_owned = my_count + enemy_count
    if total_owned == 0:
        return EVEN, 0.5
    power = my_count / total_owned
    prod_power = my_prod / max(1, my_prod + enemy_prod)
    ship_power = my_ships / max(1, my_ships + enemy_ships)
    combined = (power + prod_power + ship_power) / 3.0
    if step < 30:
        return EVEN, combined
    if combined > 0.55:
        return DOMINANT, combined
    if combined > 0.35:
        return EVEN, combined
    if combined > 0.20:
        return BEHIND, combined
    return DESPERATE, combined


def get_custom_score(m, t, situation=EVEN, m_ships=None):
    ...
    if situation == BEHIND:
        if t.owner != -1 and t.ships > 25:
            score -= 200
        if t.owner == -1:
            score += 10
    elif situation == DESPERATE:
        if t.owner != -1 and t.ships > 15:
            score -= 300
        if t.owner == -1:
            score += 15

    if m_ships is not None and total_ships > m_ships:
        score -= 100
...
def agent(obs):
    ...
    situation, power = assess_situation(mine, targets, step_now)

    if situation == BEHIND:
        min_garrison_to_attack = 20
    elif situation == DESPERATE:
        min_garrison_to_attack = 40
    else:
        min_garrison_to_attack = MIN_SHIPS_MINE_ATTACK

    remaining_steps = 500 - step_now
    ...
    for m in sorted(lobs.get("mine", []), key=lambda p: p.ships, reverse=True):
        ...
        if m.ships < min_garrison_to_attack:
            continue
        ...
        for t in lobs.get("targets", []):
            ...
            score = get_custom_score(m, t, situation, m.ships)
            candidate_targets.append((m, t, score))

        candidate_targets = sorted(candidate_targets, key=lambda x: x[2], reverse=True)

        n_candidates = 3 if situation <= EVEN else 2

        for m, t, s in candidate_targets[:n_candidates]:
            ...
            if m_available_ships >= base_ships: # single attack
                # v131: concentrated strike — when garrison is large and safe,
                # send all available ships as one big fleet instead of the minimum.
                # Boosts max_fleet (ours 140 vs 900+ 200+) and avg_fleet.
                send_ships = base_ships
                if m.id not in under_attack and m_available_ships >= 40:
                ...
                if angle is not None and arrive_tick is not None:
                    if arrive_tick > remaining_steps:
                        continue
