...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            if tgt.owner != -1:
                base_value += tgt.production * time_owned
            # v51: in 4P, push toward center — top players drift -8, we drift
            # -1.7. Modest bonus weights central targets slightly higher in 4P.
            # 2P unchanged because v44's center bonus hurt 2P games.
            if num_players == 4:
                tgt_dist_to_center = math.hypot(tgt.x - CENTER, tgt.y - CENTER)
                base_value *= 1.0 + 0.25 / (tgt_dist_to_center + 5.0)
