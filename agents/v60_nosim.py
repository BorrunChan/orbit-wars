...
def agent(obs):
    ...
    # v60: NO sim validation. Pure heuristic + shot filter. Accept top
    # candidates by score, with constraints (one per target, respect max_launch).
    accepted = []
    ...
    for key in group_keys:
        ...
        if tid in targeted:
            continue
        # Pick best variant within group (prefer minimum capture to save ships)
        best_variant = None
        for score, _mid, _tid, ships, angle, T in sorted(grouped[key],
                                                          key=lambda c: c[3]):
