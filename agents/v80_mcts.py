...
def agent(obs):
    ...
    # v80: MCTS allocation instead of greedy. Cap candidates at K to bound
    # tree branching. UCB1 selection, sim rollouts at leaves.
    ...
    # Dedup by (mid, tid) — keep best variant per group
    seen_pairs = set()
    deduped = []
    ...
    accepted = _mcts.search(
        ...
        opp_starter_fn=_opp_starter_moves,
        iterations=80,   # bumped from 30 — better coverage of action space
        max_depth=6,
    )
    return accepted
