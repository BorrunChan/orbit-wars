...
SHOT_REJECT_THRESHOLD = 0.40  # P(success) — reject candidates below this
...
SUN_BUFFER = 0.6
EPISODE_STEPS = 500
SIM_HORIZON = 16     # default; agent picks per map
SIM_HORIZON_SLOW = 22  # slow rot maps: deeper sim
SIM_HORIZON_FAST = 12  # fast rot: shorter sim
DEFENSE_HORIZON = 40
...
def agent(obs):
    ...
    # === Shot-reject score blend (from learned classifier) ===
    # For each candidate, classifier predicts P(this fleet captures target).
    # Soft-blend: multiply original score by P(success)^0.5. Bad shots get
    # downweighted in the sort but not removed; sim greedy then chooses.
    # Plus: REJECT only the very worst (P < 0.10).
    if _SHOT_TREES is not None:
        ...
        for c in candidates:
            ...
            if p_success >= SHOT_REJECT_THRESHOLD:
                filtered.append(c)
        candidates = filtered

    candidates.sort(reverse=True, key=lambda c: c[0])

    # === Map-conditional sim depth ===
    # Slow rotation -> deeper sim (predictions stay accurate longer).
    # Fast rotation -> shorter sim (long predictions drift too much).
    if angular_velocity < 0.035:
        sim_horizon = SIM_HORIZON_SLOW
        k_cap = 20
    else:
