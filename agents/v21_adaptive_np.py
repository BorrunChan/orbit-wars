...
SUN_BUFFER = 0.6
EPISODE_STEPS = 500
SIM_HORIZON_2P = 16  # turns to look ahead in 2P games
SIM_HORIZON_4P = 8   # shorter in 4P (more variance, less reliable sim)
DEFENSE_HORIZON = 40
...
# Threshold is parameterized so we can tune separately for 2P vs 4P.
_OPP_THRESH = [20]  # mutable wrapper; default v3p value


def _opp_starter_moves(state, opp_player):
    """Approximate starter's decision: each mine with >=_OPP_THRESH ships
    sends half toward the closest static unowned target."""
    thresh = _OPP_THRESH[0]
    moves = []
    for p in state["planets"]:
        if p[1] != opp_player or p[5] < thresh:
            continue
        x0, y0 = p[2], p[3]
...
def agent(obs):
    ...
    # Per-game-type params (2P vs 4P). 2P values are random-search-tuned.
    if num_players >= 4:
        _OPP_THRESH[0] = 20
        T_w = 0.5
    else:
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            candidates.append((base_value / (min_ships + T_w * T),
            ...
            if buf > 0:
                ...
                candidates.append((base_value / (big_ships + T_w * T) * 0.95,
                                    mine.id, tgt.id, big_ships, angle, T))

    candidates.sort(reverse=True, key=lambda c: c[0])

    # Adapt horizon and K to game size. 2P benefits from longer horizon and
    # tighter candidate set (clearer winner per group). 4P has more variance
    # and shorter horizon avoids noise.
    if num_players >= 4:
        SIM_HORIZON = SIM_HORIZON_4P
        K_cap = 30
    else:
        SIM_HORIZON = SIM_HORIZON_2P
        K_cap = 15
