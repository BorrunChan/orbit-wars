"""
v124 + instrumentation. Writes per-turn trace JSONL to /tmp/v124_trace.jsonl
to diagnose where v124 is losing on proto1000. NOT for deployment.
"""
...
_TRACE_PATH = os.environ.get("V124_TRACE_PATH", "/tmp/v124_trace.jsonl")


def _trace_emit(rec, truncate=False):
    try:
        mode = "w" if truncate else "a"
        with open(_TRACE_PATH, mode) as f:
...
def agent(obs):
    ...
    max_launch = {}
    _trace_per_planet = []  # (id, ships, safe, floor, max_launch_final, threats_n, threats_ships)
    for mine in my_planets:
        ...
        _trace_per_planet.append({
            "id": mine.id, "ships": mine.ships, "prod": mine.production,
            "safe": safe, "floor": floor_val,
            "ml_final": max_launch[mine.id],
            "floor_bound": max_launch[mine.id] > safe,
            "threats_n": len(thr),
            "threats_ships": sum(s for _, s in thr),
        })
    ...
    _trace_cand_pre = len(candidates)
    ...
    _trace_shot_rejected = 0
    _trace_swarm_rescued = 0
    if _SHOT_TREES is not None:
        ...
        for c in candidates:
            ...
            if p_success >= shot_threshold:
                ...
                filtered.append(c)
                _trace_swarm_rescued += 1
            else:
                _trace_shot_rejected += 1
        candidates = filtered

    candidates.sort(reverse=True, key=lambda c: c[0])
    _trace_cand_post = len(candidates)
    ...
    _trace_K_truncated = max(0, len(candidates) - K)
    _trace_truncated_max_score = (candidates[K][0]
                                   if len(candidates) > K else 0)
    grouped = {}
    ...
    group_keys = sorted(grouped.keys(),
                         ...
                         reverse=True)
    _trace_groups_visited = 0
    _trace_groups_accepted = 0
    _trace_groups_sim_rejected = 0
    _trace_sim_evals = []  # (best_eval_for_group, current_best, accepted_bool)
    for key in group_keys:
        mid, tid = key
        if tid in targeted:
            continue
        _trace_groups_visited += 1
        best_variant = None
        ...
        if best_variant is not None:
            ...
            used[mid] += ships
            targeted.add(tid)
            _trace_groups_accepted += 1
            _trace_sim_evals.append({"tid": tid, "delta": best_eval_for_group - baseline_eval, "accepted": True})
        else:
            _trace_groups_sim_rejected += 1
            _trace_sim_evals.append({"tid": tid, "delta": 0, "accepted": False})
    ...
    _V123_MY_LAUNCH_TOTAL += len(accepted)

    # --- TRACE EMIT ---
    # v123 raw diagnostics
    _v123_opp_total = max(0,
        (next_fleet_id_now - (_V123_INIT_NFID or 0)) - _V123_MY_LAUNCH_TOTAL)
    _v123_rate = _v123_opp_total / max(1, step_now)
    _trace_rec = {
        "step": step_now,
        "num_players": num_players,
        "player": player,
        "next_fleet_id": next_fleet_id_now,
        "v123_init_nfid": _V123_INIT_NFID,
        "v123_my_launches": _V123_MY_LAUNCH_TOTAL,
        "v123_opp_total": _v123_opp_total,
        "v123_rate": _v123_rate,
        "my_planets_n": len(my_planets),
        "my_garrison": sum(p.ships for p in my_planets),
        "my_prod": sum(p.production for p in my_planets),
        "opp_planets_n": sum(1 for p in planets
                              if p.owner != player and p.owner != -1),
        "opp_garrison": sum(p.ships for p in planets
                             if p.owner != player and p.owner != -1),
        "neutral_planets": sum(1 for p in planets if p.owner == -1),
        "enemy_fleets_n": len(_enemy_fleets),
        "enemy_fleets_ships": sum(f.ships for f in _enemy_fleets),
        "n_small_inflight": _n_small,
        "extreme_swarm": extreme_swarm,
        "early_swarm_2p": early_swarm_2p,
        "swarm_active_v95": swarm_active,
        "v123_swarmer_confirmed": v123_swarmer_confirmed,
        "shot_threshold_final": shot_threshold,
        "per_planet": _trace_per_planet,
        "cap_cost_none": _trace_cap_cost_none,
        "angle_none": _trace_angle_none,
        "cand_pre_shot": _trace_cand_pre,
        "cand_post_shot": _trace_cand_post,
        "shot_rejected": _trace_shot_rejected,
        "swarm_rescued": _trace_swarm_rescued,
        "K_truncated": _trace_K_truncated,
        "trunc_max_score": _trace_truncated_max_score,
        "baseline_eval": baseline_eval,
        "current_best_final": current_best,
        "eval_delta": current_best - baseline_eval,
        "groups_visited": _trace_groups_visited,
        "groups_accepted": _trace_groups_accepted,
        "groups_sim_rejected": _trace_groups_sim_rejected,
        "accepted_n": len(accepted),
        "accepted_ships": sum(int(a[2]) for a in accepted),
        "sim_evals": _trace_sim_evals,
        "floor_binding_n": sum(1 for p in _trace_per_planet if p["floor_bound"]),
    }
    _trace_emit(_trace_rec, truncate=(step_now <= 1))
