...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            if tgt.owner != -1:
                base_value += tgt.production * time_owned
            # v78: in 4P, score = base_value^2 / cost (super-emphasize high
            # production targets). In 2P, keep linear formula.
            if num_players == 4:
                score1 = base_value * base_value / max(1, (min_ships + T) * 100)
            else:
                score1 = base_value / (min_ships + 1.0 * T)
            candidates.append((score1, mine.id, tgt.id, min_ships, angle, T))
            buf = min(int(min_ships * 0.5) + 5, avail - min_ships)
            if buf > 0:
                big_ships = min_ships + buf
                if num_players == 4:
                    score2 = base_value * base_value / max(1, (big_ships + T) * 100) * 0.95
                else:
                    score2 = base_value / (big_ships + 1.0 * T) * 0.95
                candidates.append((score2, mine.id, tgt.id, big_ships, angle, T))
