...
SHOT_REJECT_THRESHOLD = 0.40   # fallback default
SHOT_THRESHOLD_SLOW = 0.20     # fallback default

# --- Map cluster dispatcher ---
# Load cluster centers + per-cluster preset params from a JSON dump.
# Each cluster has: feature center (10-d), and a set of preset
# {shot_threshold, sim_horizon, k_cap, ...} learned offline from
# round-robin win data.
# If cluster_params.json is missing, fall back to v35 logic.
_CLUSTER_PARAMS = None
try:
    import json as _json
    import os as _os
    _params_path = None
    # Look for cluster_params.json next to this file (Kaggle exec adds dir to path)
    for d in ("/kaggle_simulations/agent", "/tmp", "."):
        p = _os.path.join(d, "cluster_params.json")
        if _os.path.exists(p):
            _params_path = p
            break
    if _params_path:
        with open(_params_path) as _f:
            _CLUSTER_PARAMS = _json.load(_f)
except Exception:
    pass


def _map_feature_vector(obs):
    """10-dim feature vector matching scripts/cluster_maps.py."""
    planets = obs.get("planets", []) if isinstance(obs, dict) else getattr(obs, "planets", [])
    ang_vel = obs.get("angular_velocity", 0) if isinstance(obs, dict) else getattr(obs, "angular_velocity", 0)
    ...
    if homes and neutrals:
        h = homes[0]
        dist_home = min(math.hypot(h[2]-n[2], h[3]-n[3]) for n in neutrals)
    else:
        dist_home = 0
    avg_neutral = sum(p[5] for p in neutrals) / max(1, n_neutrals)
    max_neutral = max((p[5] for p in neutrals), default=0)
    max_prod = max((p[6] for p in neutrals), default=0)
    from collections import Counter
    prod_counter = Counter(p[6] for p in neutrals)
    total = sum(prod_counter.values()) or 1
    entropy = 0
    for c in prod_counter.values():
        pr = c / total
        if pr > 0:
            entropy -= pr * math.log(pr)
    high_prod = sum(v for k, v in prod_counter.items() if k >= 3)
    low_prod = sum(v for k, v in prod_counter.items() if k <= 2)
    return [
        ang_vel * 25,
        rotating / max(1, n_planets),
        avg_neutral / 50.0,
        max_neutral / 99.0,
        max_prod / 5.0,
        dist_home / 30.0,
        entropy,
        high_prod / max(1, n_planets),
        low_prod / max(1, n_planets),
        n_planets / 40.0,
    ]


def _classify_cluster(obs):
    """Assign current map to nearest cluster center. Returns cluster index or None."""
    if _CLUSTER_PARAMS is None:
        return None
    feats = _map_feature_vector(obs)
    mean = _CLUSTER_PARAMS["feature_mean"]
    std = _CLUSTER_PARAMS["feature_std"]
    centers = _CLUSTER_PARAMS["centers_normalized"]
    # Z-score normalize same as training
    norm = [(feats[i] - mean[i]) / max(1e-9, std[i]) for i in range(len(feats))]
    best = None; best_d = float("inf")
    for c, center in enumerate(centers):
        d = sum((norm[i] - center[i]) ** 2 for i in range(len(norm)))
        if d < best_d:
            best_d = d; best = c
    return best


def _params_for_cluster(c):
    """Per-cluster preset. Falls back to v35 defaults if no params for c."""
    if _CLUSTER_PARAMS is None or c is None:
        return None
    preset = _CLUSTER_PARAMS.get("cluster_presets", {}).get(str(c))
    return preset  # dict with keys: shot_threshold, sim_horizon, k_cap
...
def agent(obs):
    ...
    # === Map-cluster dispatch ===
    # Try cluster classifier first (10-dim feature → nearest cluster center).
    # Each cluster has preset (shot_threshold, sim_horizon, k_cap).
    # If cluster params missing, fall back to v35 logic (angular_velocity split).
    cluster_id = _classify_cluster(obs)
    preset = _params_for_cluster(cluster_id) if cluster_id is not None else None
    if preset is not None:
        shot_threshold = preset.get("shot_threshold",
                                     SHOT_THRESHOLD_SLOW if angular_velocity < 0.035
                                     else SHOT_REJECT_THRESHOLD)
        sim_horizon = preset.get("sim_horizon", 16)
        k_cap = preset.get("k_cap", 15)
    else:
