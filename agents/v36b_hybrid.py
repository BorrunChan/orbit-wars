...
# --- Hybrid map dispatch ---
# v35's av-only split is good for most cases. v36b adds: if the map
# looks like cluster 2 (where I dominate at 79%), force loose threshold
# even if av is fast. Cluster identification is by 10-d feature center.
...
def _classify_cluster(obs):
    ...
    feats = [
        ang_vel * 25, rotating / max(1, n_planets), avg_neutral / 50.0,
        max_neutral / 99.0, max_prod / 5.0, dist_home / 30.0,
        entropy, high_prod / max(1, n_planets),
        low_prod / max(1, n_planets), n_planets / 40.0,
    ]
...
def agent(obs):
    ...
    # === Hybrid map-conditional threshold ===
    # Base rule (from v35): av < 0.035 → loose (0.20), else tight (0.40).
    # ADDITION: if map clusters into cluster 2 (where main wins 79%),
    # FORCE loose threshold regardless of av (cluster 2 spans both av ranges).
    base = (SHOT_THRESHOLD_SLOW if angular_velocity < 0.035
            else SHOT_REJECT_THRESHOLD)
    cluster = _classify_cluster(obs)
    if cluster == 2:
        shot_threshold = SHOT_THRESHOLD_SLOW   # my dominant cluster
    elif cluster == 0:
        # mlhybrid 87% kingdom — tighten more than av rule
        shot_threshold = max(base, 0.50)
    else:
        shot_threshold = base
