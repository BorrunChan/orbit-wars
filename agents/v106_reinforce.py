...
def agent(obs):
    ...
    # v106: PROACTIVE REINFORCEMENT — for doomed planets (will fall with zero
    # dispatch), find nearest mine that can send reinforcement arriving before
    # failing_turn. v98 only USES _doom_deficit to filter (max_launch=0,
    # planet self-quarantines); proto1000 actively dispatches saviors.
    reinforcement_moves = []
    for mp_d in my_planets:
        ...
        send_ships = int(deficit_ships * 1.2) + 3  # 20% buffer
        ...
        for src in by_dist:
            ...
            if max_launch[src.id] < send_ships:
                continue  # src can't afford after its own threats
            ...
            break  # one reinforcement per doomed planet
