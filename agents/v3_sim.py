...
def _simulate(state, my_actions, opp_actions, horizon, player, opp_model="starter"):
    """Apply this turn's actions, then run `horizon` turns. opp_model decides
    what we expect the opponent to do each future turn:
      - "starter": send half-ship fleets at closest static targets
    ...
    for _ in range(horizon - 1):
        if opp_model == "starter":
            opp_moves = _opp_starter_moves(state, 1 - player)
        else:
            opp_moves = []
        actions = [None, None]
        actions[player] = []
        actions[1 - player] = opp_moves
        sim.step(state, actions)
