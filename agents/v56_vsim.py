...
def _v_eval_state(state, player, num_players):
    """v56: use V model logit as sim eval (replace ships diff heuristic)."""
    planets = [Planet(*p) for p in state["planets"]]
    fleets = [Fleet(*f) for f in state["fleets"]]
    feats = _v_feature_vector(planets, fleets, player, num_players,
                                state["step"])
    z = _V_BASE
...
def _simulate(state, my_actions, opp_actions, horizon, player, num_players=2,
    ...
    for _ in range(horizon - 1):
        ...
        sim.step(state, actions)
    # v56: prefer V if loaded; else fall back to ships-diff heuristic
    if _V_TREES is not None:
        return _v_eval_state(state, player, num_players)
