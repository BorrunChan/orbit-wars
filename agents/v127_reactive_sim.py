...
def _opp_retake_moves(state, opp_player, just_captured):
    """v127: REACTIVE opp model. Unlike starter/high_volume (which target the
    nearest non-self planet regardless of my actions), this model first sends
    a retake force at planets the player JUST captured this step — enough to
    overcome garrison + production over flight + margin. This breaks the
    baseline/trial symmetry that made v115/v116 byte-identical: a trial state
    where I just captured X gets specifically punished, so the sim correctly
    devalues flimsy captures that a real swarmer would retake.

    Remaining ships fall back to high-volume expansion at nearest non-self.
    """
    planets = state["planets"]
    pid_to_planet = {p[0]: p for p in planets}
    opp_planets = [p for p in planets if p[1] == opp_player]
    used = {p[0]: 0 for p in opp_planets}
    moves = []

    # Priority 1: retake just-captured planets
    for tid in just_captured:
        tgt = pid_to_planet.get(tid)
        if tgt is None or tgt[1] == opp_player:
            continue
        tx, ty = tgt[2], tgt[3]
        best = None
        best_d = float('inf')
        ...
        if best is None:
            continue
        spd = _speed(best[5])
        T = max(1, math.ceil(best_d / spd))
        needed = tgt[5] + tgt[6] * T + 2
        free = best[5] - used[best[0]]
        send = min(free, needed)
        if send < 5:
            continue
        angle = math.atan2(ty - best[3], tx - best[2])
        moves.append([best[0], angle, send])
        used[best[0]] += send

    # Priority 2: high-volume expansion with remaining ships
...
def _simulate(state, my_actions, opp_actions, horizon, player, num_players=2,
    ...
    what we expect the opponent to do each future turn:
      ...
      - "retake":  reactive — retake my just-captured planets, then expand
    ...
    prev_owners = {p[0]: p[1] for p in state["planets"]}
    ...
    for _ in range(horizon - 1):
        ...
        elif opp_model == "retake":
            just_captured = [p[0] for p in state["planets"]
                             if p[1] == player
                             and prev_owners.get(p[0]) != player]
            for opp in range(num_players):
                if opp == player:
                    continue
                opp_moves = _opp_retake_moves(state, opp, just_captured)
                if opp_moves:
                    actions[opp] = opp_moves
        prev_owners = {p[0]: p[1] for p in state["planets"]}
        sim.step(state, actions)
...
def agent(obs):
    ...
    # v127: when 2P swarm detected, switch to "retake" — reactive model that
    # punishes flimsy captures the swarmer would retake. Gated to swarm_active
    # so structured opps (who don't retake) keep the calibrated starter model.
    if num_players == 4:
        sim_opp_model = "none"
    elif swarm_active:
        sim_opp_model = "retake"
    else:
        sim_opp_model = "starter"
