...
def agent(obs):
    ...
    # v109: in 2P, drastically lower shot_threshold to let high-volume
    # launches through. proto1000 doesn't use a filter at all (MIN_SHIPS=5
    # regardless of P(success)) and wins 6/6 vs v98.
    if num_players == 2:
        shot_threshold = min(shot_threshold, 0.10)
    ...
    # v109: increased K from 15 to 40 in 2P, to expose more candidates to
    # sim greedy. Hypothesis: v98 limits volume by truncating candidates list
    # before greedy. proto1000 wins via 10x launches; bigger K lets more
    # launches through.
    # Group candidates by (mid, tid). For each (mid, tid), present both
    # variants (min and buffered) and let sim pick whichever — or neither.
    K = min(40 if num_players == 2 else 15, len(candidates))
    grouped = {}
