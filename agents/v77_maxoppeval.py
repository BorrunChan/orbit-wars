...
def _simulate(state, my_actions, opp_actions, horizon, player, num_players=2,
    ...
    for _ in range(horizon - 1):
        ...
        sim.step(state, actions)
    # v77: in 4P, eval focuses on strongest opp (not sum of 3). 2P unchanged.
    if num_players == 4:
        my_s = my_p = 0
        opp_s_per = [0, 0, 0, 0]
        opp_p_per = [0, 0, 0, 0]
        for pl in state["planets"]:
            ...
            if o == player:
                ...
                opp_s_per[o] += pl[5]; opp_p_per[o] += pl[6]
        for f in state["fleets"]:
            if f[1] == player:
                my_s += f[6]
            elif f[1] != -1:
                opp_s_per[f[1]] += f[6]
        max_opp_s = max(opp_s_per[i] for i in range(4) if i != player)
        max_opp_p = max(opp_p_per[i] for i in range(4) if i != player)
        remaining = max(1, 500 - state["step"])
        return (my_s - max_opp_s) + (my_p - max_opp_p) * remaining * 0.3
