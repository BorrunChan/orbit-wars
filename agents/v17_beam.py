...
def agent(obs):
    ...
    # Group candidates by (mid, tid). Each group offers up to 2 variants.
    ...
    # Beam search over plans. Each beam element: (moves, used_per_mine,
    # targeted_set, eval). Width B; we expand each by trying the next group.
    B = 3

    initial_used = {p.id: 0 for p in my_planets}
    beams = [(list(), dict(initial_used), set(), baseline_eval)]

    for key in group_keys:
        mid, tid = key
        new_beams = []
        for moves, used, targeted, ev in beams:
            # Option 1: skip this group (don't attack tid from mid)
            new_beams.append((moves, used, targeted, ev))
            if tid in targeted:
                continue
            # Option 2: try each variant; choose the BEST variant for this beam
            best = None
            for score, _mid, _tid, ships, angle, T in grouped[key]:
                if max_launch[mid] - used.get(mid, 0) < ships:
                    continue
                trial_moves = moves + [[mid, angle, int(ships)]]
                trial_state = sim.clone(sim_state_base)
                new_eval = _simulate(trial_state, trial_moves, [], SIM_HORIZON,
                                      player, num_players=num_players,
                                      opp_model="starter")
                if best is None or new_eval > best[3]:
                    new_used = dict(used)
                    new_used[mid] = new_used.get(mid, 0) + ships
                    new_targeted = set(targeted)
                    new_targeted.add(tid)
                    best = (trial_moves, new_used, new_targeted, new_eval)
            if best is not None:
                new_beams.append(best)
        # Prune beams to top B by eval
        new_beams.sort(key=lambda b: b[3], reverse=True)
        beams = new_beams[:B]

    # Return the moves from the best beam
    best_beam = beams[0]
    return best_beam[0]
