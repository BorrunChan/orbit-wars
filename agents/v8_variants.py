...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            if tgt.owner != -1:
                base_value += tgt.production * time_owned
            # Multiple ship-count variants for sim to evaluate.
            seen_sizes = set()
            for ships in (min_ships,
                          min_ships + 5,
                          min_ships + 15,
                          min(avail, int(min_ships * 2)),
                          min(avail, int(min_ships * 3))):
                ships = int(ships)
                if ships < min_ships or ships > avail or ships in seen_sizes:
                    continue
                seen_sizes.add(ships)
                # Recompute T for this fleet size (bigger = faster)
                fleet_T = max(1, math.ceil(
                    math.hypot(arrival[0] - mine.x, arrival[1] - mine.y) /
                    _speed(ships)))
                score = base_value / (ships + 0.5 * fleet_T)
                # Slight discount for non-minimum (sim still picks if useful)
                if ships > min_ships:
                    score *= 0.92
                candidates.append((score, mine.id, tgt.id, ships, angle, fleet_T))
