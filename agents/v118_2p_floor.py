...
def agent(obs):
    ...
    # v118: 2P expansion-deficit also triggers floor — diagnosed v98 losing
    # 12/15 vs proto1000 because 2P never forced launch floor; defense too
    # tight when behind, opponent free-expands to elimination by step 109-209
    _opp_planet_count_early = sum(
        1 for p in planets if p.owner != player and p.owner != -1
    )
    _my_planet_count_early = len(my_planets)
    _expansion_deficit_early = (
        _opp_planet_count_early > _my_planet_count_early + 1
    )
    extreme_swarm = (
        (num_players == 4 and (_n_small >= 20 or _n_total >= 25))
        or (num_players == 2 and _expansion_deficit_early)
    )
