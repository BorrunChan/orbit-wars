...
def agent(obs):
    ...
    # Anti-hoarding fallback: if sim accepted nothing but heuristic has good
    # candidates, force-launch the top viable ones. Sim's "do nothing baseline"
    # can underestimate the cost of inaction against aggressive opponents.
    if not accepted:
        used_anti = {p.id: 0 for p in my_planets}
        targeted_anti = set()
        for score, mid, tid, ships, angle, T in candidates[:8]:
            if tid in targeted_anti:
                continue
            if max_launch[mid] - used_anti[mid] < ships:
                continue
            accepted.append([mid, angle, int(ships)])
            used_anti[mid] += ships
            targeted_anti.add(tid)
            if len(accepted) >= 3:
                break
