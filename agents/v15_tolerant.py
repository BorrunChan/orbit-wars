...
def agent(obs):
    ...
    for key in group_keys:
        ...
        if tid in targeted:
            continue
        best_variant = None
        # Tolerance: accept candidates that don't drop eval by more than TOL.
        # The sim's eval is noisy (opp model is approximate), so a strict
        # ">= current_best" rejects too aggressively. Half-trust the heuristic.
        TOL = 25.0
        best_eval_for_group = current_best - TOL
