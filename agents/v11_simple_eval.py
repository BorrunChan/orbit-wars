...
def _simulate(state, my_actions, opp_actions, horizon, player, num_players=2,
    ...
    for _ in range(horizon - 1):
        ...
        sim.step(state, actions)
    return _eval_v11(state, player)


def _eval_v11(state, player):
    """Eval emphasizing planet ownership + ship count, less prod-weighted."""
    my_ships = opp_ships = 0
    my_pl = opp_pl = 0
    for p in state["planets"]:
        if p[1] == player:
            my_ships += p[5]; my_pl += 1
        elif p[1] != -1:
            opp_ships += p[5]; opp_pl += 1
    for f in state["fleets"]:
        if f[1] == player:
            my_ships += f[6]
        else:
            opp_ships += f[6]
    # Strongly value planet ownership delta
    return (my_pl - opp_pl) * 100 + (my_ships - opp_ships)
