...
def agent(obs):
    ...
    # v120: FORCED EXPANSION in 2P early race window (step < 20).
    # Diagnosis: sim greedy with "starter" opp_model thinks "no-action" is
    # safe in early steps (opp model requires 20 ships before launching).
    # Reality: proto1000-class opponents expand to 9 planets by step 30.
    # Fix: after sim greedy, for each my_planet with unused ships, try the
    # nearest unowned target. Only launch if _capture_cost confirms the
    # planet is *actually capturable* with available ships (avoids the
    # v117 dead-end of "5-ship pressure that can't capture anything").
    if num_players == 2 and step_now < 20:
        for mine in my_planets:
            remaining = max_launch.get(mine.id, 0) - used.get(mine.id, 0)
            if remaining < 5:
                continue
            # Only target genuinely capturable neutrals
            neut = [t for t in targets
                    if t.owner == -1 and t.id not in targeted]
            if not neut:
                continue
            nearest = min(neut, key=lambda t: math.hypot(t.x - mine.x,
                                                          t.y - mine.y))
            est = _capture_cost(mine.x, mine.y, nearest,
                                 (nearest.x, nearest.y), remaining, step_now,
                                 initial_by_id, angular_velocity,
                                 comet_groups, player)
            if est is None:
                continue
            min_ships, T, arrival = est
            if min_ships > remaining:
                continue
            angle = _safe_launch_angle((mine.x, mine.y), arrival,
                                        nearest.radius)
            if angle is None:
                continue
            accepted.append([mine.id, angle, int(min_ships)])
            used[mine.id] += min_ships
