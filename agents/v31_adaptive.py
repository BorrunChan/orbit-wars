...
SUN_BUFFER = 0.6
EPISODE_STEPS = 500
SIM_HORIZON = 16     # default; agent() overrides per map rotation speed
SIM_HORIZON_SLOW = 22  # slow rot maps reward deeper sim (lb1224 style)
SIM_HORIZON_FAST = 10  # fast rot: shorter sim, more reactive
DEFENSE_HORIZON = 40
...
def agent(obs):
    ...
    # === Map-conditional sim depth (from round-robin analysis) ===
    # lb1224 wins 80% on slow-rot maps with deep sim, only 50% on fast-rot.
    # mlhybrid is opposite (78% fast / 60% slow). My current v20 is 55% on
    # both — picking depth per map could close the gap.
    if angular_velocity < 0.035:
        sim_horizon = SIM_HORIZON_SLOW   # 22
        k_cap = 25
    else:
        sim_horizon = SIM_HORIZON_FAST   # 10
        k_cap = 15
