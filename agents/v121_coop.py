...
def agent(obs):
    ...
    # v121: COOP ATTACK. proto1000 uses up to 8 planets to coop-attack
    # high-garrison targets that single planet can't capture (MIN_TARGET=20).
    # v98 only does single-planet capture, leaving high-value enemy planets
    # untouched. Bench showed v98 vs proto1000 = 3/15 because proto1000
    # expands faster and v98 cannot reduce its planet count.
    # Sim-validated: every coop attempt is tried in a sim rollout; commits
    # only if score improves.
    if num_players == 2 and step_now >= 3:
        for tgt in targets:
            ...
            candidates_mp = []
            for mine in my_planets:
                rem = max_launch.get(mine.id, 0) - used.get(mine.id, 0)
                if rem < 5:
                    continue
                dist = math.hypot(mine.x - tgt.x, mine.y - tgt.y)
                candidates_mp.append((mine, rem, dist))
            if len(candidates_mp) < 2:
                continue
            candidates_mp.sort(key=lambda x: x[2])
            picked = []
            total_ships = 0
            max_eta = 0.0
            for mine, rem, dist in candidates_mp[:8]:
                angle = _safe_launch_angle((mine.x, mine.y),
                                            (tgt.x, tgt.y), tgt.radius)
                if angle is None:
                    continue
                send = min(rem, max(5, int(tgt.ships * 0.5)))
                eta = dist / _speed(max(1, send))
                picked.append((mine, send, angle))
                total_ships += send
                if eta > max_eta:
                    max_eta = eta
                needed = tgt.ships + 1
                if tgt.owner != -1:
                    needed += int(max_eta * tgt.production) + 2 * len(picked)
            ...
            if total_ships < needed:
                continue
            trial_moves = list(accepted)
            for mine, send, angle in picked:
                trial_moves.append([mine.id, angle, int(send)])
            trial_state = sim.clone(sim_state_base)
            new_eval = _simulate(trial_state, trial_moves, [], sim_horizon,
                                  player, num_players=num_players,
                                  opp_model=sim_opp_model)
            if new_eval > current_best:
                for mine, send, angle in picked:
                    accepted.append([mine.id, angle, int(send)])
                    used[mine.id] += send
                targeted.add(tgt.id)
