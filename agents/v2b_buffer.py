"""
Orbit Wars - Agent v2b

v1 + post-capture survival buffer: when launching, send slightly more than
the bare-minimum capture force so the freshly captured planet can survive
the first enemy raid.
"""
...
def _survival_buffer(target):
    """Extra ships to send beyond the capture minimum, so the freshly
    captured planet can absorb one round of enemy counter-attack."""
    # High-production planets are juicy targets — pad more.
    return max(3, int(2 * target.production) + (1 if target.owner != -1 else 0))
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            if est is None:
                continue
            ships, T, arrival = est
            # Add survival buffer if affordable.
            buf = _survival_buffer(tgt)
            ships_with_buf = min(mine.ships, ships + buf)
            ...
            if tgt.owner != -1:
                value += tgt.production * time_owned
            score = value / (ships_with_buf + 0.5 * T)
            candidates.append((score, mine.id, tgt.id, ships_with_buf, angle, T))
