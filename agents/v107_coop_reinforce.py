...
def agent(obs):
    ...
    # v106: PROACTIVE REINFORCEMENT (stacked on top of v105 coop attack).
    # For doomed planets, find nearest mine to send reinforcement before
    # failing_turn. Reduces max_launch on the source mine.
    reinforcement_moves = []
    for mp_d in my_planets:
        ...
        by_dist = sorted(my_planets,
                          key=lambda m: math.hypot(m.x - mp_d.x,
                                                     m.y - mp_d.y))
        for src in by_dist:
            if src.id == mp_d.id:
                continue
            ...
            if dist < 1e-6:
                continue
            speed = _speed(send_ships)
            eta = max(1, math.ceil(dist / speed))
            if eta >= failing_turn:
                continue
            if max_launch[src.id] < send_ships:
                continue
            pred = _predict_position(mp_d.id, (mp_d.x, mp_d.y),
                                      initial_by_id, angular_velocity,
                                      step_now, step_now + eta, comet_groups)
            if pred is None:
                continue
    ...
    # v105: COOP ATTACK candidates — when 2 mines together can capture a
    # target that no single mine can. Inspired by proto1000's coop pattern
    # (v98 vs proto1000 = 0/6 in 2P; coop is the key missing capability).
    # Uses the 2 nearest mines to the target with available ships.
    coop_candidates = []
    for tgt in targets:
        # Focus on enemy planets — neutrals are usually captured by single mine
        if tgt.owner == -1:
            continue
        if tgt.production < 1:
            continue
        # Mines with budget >= 5 are eligible to contribute
        contrib = [(m, max_launch[m.id]) for m in my_planets
                    if max_launch[m.id] >= 5]
        if len(contrib) < 2:
            continue
        contrib.sort(key=lambda mb: math.hypot(mb[0].x - tgt.x,
                                                 mb[0].y - tgt.y))
        m1, b1 = contrib[0]
        m2, b2 = contrib[1]
        combined = b1 + b2
        # Cost estimate using m1 (closest) as reference
        est = _capture_cost(m1.x, m1.y, tgt, (tgt.x, tgt.y), combined,
                             step_now, initial_by_id, angular_velocity,
                             comet_groups, player)
        if est is None:
            continue
        min_ships, T, arrival = est
        # Skip if single mine alone could already capture
        if min_ships <= max(b1, b2):
            continue
        # Skip if combined still insufficient
        if min_ships > combined:
            continue
        # Split: m1 (closer) contributes up to its budget, m2 fills the rest
        ships_1 = min(b1, max(5, min_ships - 5))
        ships_2 = min_ships - ships_1
        if ships_2 < 5 or ships_2 > b2:
            continue
        angle_1 = _safe_launch_angle((m1.x, m1.y), arrival, tgt.radius)
        angle_2 = _safe_launch_angle((m2.x, m2.y), arrival, tgt.radius)
        ...
        if tgt.owner != -1:
            base_value += tgt.production * time_owned
        # Slight penalty (×0.92): coop uses 2 launches, slightly wasteful
        # vs an equivalent single-mine attack (which is impossible here, but
        # still want to prefer single when both are viable for diff targets).
        score = base_value / (min_ships + 1.0 * T) * 0.92
        coop_candidates.append((score, tgt.id, T,
                                 [(m1.id, angle_1, ships_1),
                                  (m2.id, angle_2, ships_2)]))
    ...
    # v105: COOP processing — after single-mine greedy, try top coop
    # candidates. Each coop candidate is a 2-launch group. Accept only if
    # sim eval improves over current state.
    coop_candidates.sort(reverse=True, key=lambda c: c[0])
    COOP_K = 5
    for score, tid, T, group_moves in coop_candidates[:COOP_K]:
        if tid in targeted:
            continue
        # Check every contributing mine still has the required budget
        can_do = True
        for mid_c, _ang, ships in group_moves:
            if max_launch[mid_c] - used[mid_c] < ships:
                can_do = False
                break
        if not can_do:
            continue
        # Sim-validate: does adding both launches improve the eval?
        new_moves = [[mid_c, ang, int(ships)]
                      for mid_c, ang, ships in group_moves]
        trial_moves = accepted + new_moves
        trial_state = sim.clone(sim_state_base)
        new_eval = _simulate(trial_state, trial_moves, [], sim_horizon,
                              player, num_players=num_players,
                              opp_model=sim_opp_model)
        if new_eval > current_best:
            accepted = accepted + new_moves
            for mid_c, _ang, ships in group_moves:
                used[mid_c] += ships
            targeted.add(tid)
    ...
    # v106: prepend reinforcement moves so they're processed before attacks
