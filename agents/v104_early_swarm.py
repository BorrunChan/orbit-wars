...
def agent(obs):
    ...
    if num_players == 2:
        ...
        # v104: EARLY medium-swarm detection — catches 200-500 LR opps before
        # n_small reaches 6. 27-replay analysis showed medium swarmer (chiikawa
        # 423 / Igor V 407 / bgsszsa 496 / 新米 256) is our 2P weak spot
        # (4W/5L = 44%). By step 60, a medium swarmer has typically launched
        # 4-8 fleets — most still in flight = n_total >= 4 is reliable early.
        elif step_now < 60 and len(enemy_fleets) >= 4:
            swarm_active = True
            shot_threshold = max(0.16, shot_threshold - 0.08)
    else:  # 4P
        if n_small >= 15:
            swarm_active = True
