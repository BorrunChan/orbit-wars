...
# -- Opponent model: each non-self player sends min-capture from each planet --

def _opp_minimum_moves(state, opp_player):
    """v1-style aggressive: each opp planet sends min capture to closest
    non-self planet every turn. Includes rotating targets too."""
    moves = []
    for p in state["planets"]:
        if p[1] != opp_player or p[5] <= 1:
            continue
        x0, y0 = p[2], p[3]
...
def _simulate(state, my_actions, opp_actions, horizon, player, num_players=2,
    ...
    for _ in range(horizon - 1):
        ...
        for opp in range(num_players):
            if opp == player:
                continue
            opp_moves = _opp_minimum_moves(state, opp)
            if opp_moves:
