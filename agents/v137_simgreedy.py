"""
v137 — v135 + sim-validated launch acceptance.

In v135's attack loop, every candidate that passes feasibility gets launched
immediately. v137 adds: before committing each launch, sim-evaluate the
cumulative effect (all accepted moves + this new one). Only accept if it
improves the projected state — i.e., not a money-losing attack.

Cheap: O(K) sims per turn, not O(K^2). Catches targeting planets we can't
hold, wasteful coops, etc. Doesn't reorder candidates.
"""
import math, time
...
def agent(obs):
    ...
    # Sim base for cumulative validation
    sim_base = sim.make_state_from_obs(obs)
    sim_horizon = _SIM_HORIZON if num_players == 2 else 8  # shorter for 4P
    accepted_for_sim = []  # accumulates committed moves for sim validation

    def _eval_with_moves(extra_moves):
        s = sim.clone(sim_base)
        actions = [[] for _ in range(num_players)]
        actions[player_id] = list(accepted_for_sim) + list(extra_moves)
        sim.step(s, actions)
        for _ in range(sim_horizon - 1):
            sim.step(s, [[] for _ in range(num_players)])
        return sim.evaluate(s, player_id)

    baseline_eval = _eval_with_moves([])
    global _map_poor
        ...
        for m, t, s in candidate_targets[:3]:
            ...
            if m_available_ships >= base_ships: # single attack
                ...
                if angle is not None and arrive_tick is not None:
                    ...
                    # Sim-validate: only accept if cumulative improves baseline
                    if (time.time() - turn_start) * 1000 < _SIM_BUDGET_MS:
                        candidate_move = [m.id, angle, int(total_ships)]
                        try:
                            trial_eval = _eval_with_moves([candidate_move])
                            if trial_eval < baseline_eval:
                                continue  # money-losing, skip
                            baseline_eval = trial_eval
                            accepted_for_sim.append(candidate_move)
                        except Exception:
                            pass
