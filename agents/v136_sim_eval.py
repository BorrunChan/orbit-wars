"""
v136 — v135 + sim-based target verification.

Proto1000 lineage (v128/131/133/135) uses formula-based custom_score for
target selection. Top opponents (structured/lb1224/orbitbotnext) use sim-
greedy: forward-simulate each candidate, pick by future state value. Their
3000-line frameworks beat us 20% of games we should win.

This version layers sim verification on top of v135's framework:
  1. v135's custom_score generates candidate ranking (cheap pre-filter)
  2. For top K candidates per planet, run forward sim 10 steps
  3. Pick candidate with best evaluate(state) — captures positional value,
     not just immediate capture cost.

Falls back to formula-only when sim budget (1s/turn) runs low.
"""
import math, time
...
def _advance_state(state, horizon):
    """Advance sim state H steps with no actions from anyone."""
    s = sim.clone(state)
    for _ in range(horizon):
        sim.step(s, [[] for _ in range(4)])
    return s


def _sim_eval_move(base_state, mid, angle, ships, player, horizon):
    """Score this move: how much does our position improve H steps later?"""
    s = sim.clone(base_state)
    moves_per_player = [[] for _ in range(4)]
    moves_per_player[player] = [[mid, angle, int(ships)]]
    sim.step(s, moves_per_player)
    for _ in range(horizon - 1):
        sim.step(s, [[] for _ in range(4)])
    return sim.evaluate(s, player)
...
def get_custom_score(m, t, poor_map=False):
    ...
    if poor_map:
        ...
        prod_mult = FORMULA_PROD_MULT
        dist_weight = FORMULA_DIST
...
def agent(obs):
    ...
    effective_min_ships = 5 if step_now < 60 else MIN_SHIPS_MINE_ATTACK
    player_id = obs.get("player", 0)
    turn_start = time.time()

    # Build sim state once for this turn — used for candidate re-ranking
    sim_base = sim.make_state_from_obs(obs)
    baseline_eval = sim.evaluate(_advance_state(sim_base, _sim_horizon), player_id)
    global _map_poor
    if _map_poor is None:
        _map_poor = sum(p.production for p in lobs["planets"]) < 65
    poor_map = _map_poor
    ...
    for m in sorted(lobs.get("mine", []), key=lambda p: p.ships, reverse=True):
        ...
        for t in lobs.get("targets", []):
            ...
            score = get_custom_score(m, t, poor_map)
            candidate_targets.append((m, t, score))

        candidate_targets = sorted(candidate_targets, key=lambda x: x[2], reverse=True)

        # Sim re-ranking: for top K candidates, simulate H steps and pick best.
        # Skip if we've used > budget% of the 1s turn budget.
        elapsed_ms = (time.time() - turn_start) * 1000
        if elapsed_ms < _turn_budget_ms and len(candidate_targets) > 1:
            reranked = []
            for ci, (cm, ct, cs) in enumerate(candidate_targets[:_sim_top_k]):
                # Rough ships estimate matching attack-phase logic below
                needed = ct.ships + 1 + (3 * ct.production if ct.owner != -1 else 0)
                ships_try = max(effective_min_ships, needed)
                if cm.ships < ships_try:
                    reranked.append((cs, cm, ct, cs))
                    continue
                angle_try = math.atan2(ct.y - cm.y, ct.x - cm.x)
                try:
                    se = _sim_eval_move(sim_base, cm.id, angle_try, ships_try, player_id, _sim_horizon)
                    sim_advantage = se - baseline_eval
                except Exception:
                    sim_advantage = 0
                # Combine: formula score (immediate) + sim_advantage (positional)
                combined = cs + sim_advantage * 0.5
                reranked.append((combined, cm, ct, cs))
            # Keep rest of candidates in their original order
            reranked.sort(key=lambda x: x[0], reverse=True)
            candidate_targets = [(cm, ct, cs) for _, cm, ct, cs in reranked] + candidate_targets[_sim_top_k:]
