"""
v134 — v133 + logistics/regroup layer (ported from producer-hybrid-v4).

v133 baseline = v131 + 4P concentrated-strike fix (see git history).

NEW: after the attack/coop loop, idle owned planets (those that did not launch
this turn) marshal their garrison toward the frontline. Faithful scalar port of
orbit_lite/planner_core._plan_regroup:
  - pressure(planet) = sum over enemy planets of
        enemy.ships * max(0, 1 - dist/(fleet_speed(enemy.ships) * HORIZON))
    (distance-decayed reachable enemy mass; the "stress" gradient)
  - move ships from a low-pressure source to its highest-pressure reachable owned
    planet, only when the pressure gap clears DELTA_MIN, the hop arrives within
    MAX_TIME, and the source is not under attack / not just reinforced.
This is the "logistics layer" the meta model says top players spend ~32% of
moves on, and which v133's lineage lacked.
"""
import math
...
# --- regroup / logistics layer (ported from producer-hybrid-v4) ---
# horizon = window the pressure proxy looks ahead over; max_time = max hop length;
# delta_min = minimum pressure gap (in ship-mass units) to bother relocating.
REGROUP_HORIZON_2P = 18
REGROUP_HORIZON_4P = 13
REGROUP_MAX_TIME_2P = 7.0
REGROUP_MAX_TIME_4P = 6.0
...
def plan_regroup(lobs, under_attack, exhausted_planets_id, num_players, vel):
    """Pressure-gradient marshalling of idle owned planets' garrisons.

    Returns a list of ``[from_planet_id, angle, ships]`` moves and records each
    in ``fleet_trajectories``; mutates ``exhausted_planets_id``.
    """
    mine = lobs.get("mine", [])
    if len(mine) < 2:
        return []
    my_player = lobs.get("player")
    enemies = [p for p in lobs.get("planets", []) if p.owner >= 0 and p.owner != my_player]
    if not enemies:
        return []

    horizon = REGROUP_HORIZON_4P if num_players >= 4 else REGROUP_HORIZON_2P
    max_time = REGROUP_MAX_TIME_4P if num_players >= 4 else REGROUP_MAX_TIME_2P

    # pressure(planet) = distance-decayed reachable enemy mass.
    pressure = {}
    for mp in mine:
        s = 0.0
        for e in enemies:
            reach = _fleet_speed(e.ships) * horizon
            if reach <= 0:
                continue
            d = math.sqrt((mp.x - e.x) ** 2 + (mp.y - e.y) ** 2)
            decay = 1.0 - d / reach
            if decay > 0.0:
                s += e.ships * decay
        pressure[mp.id] = s

    # role mutex: a planet we reinforced this turn must not also be drained.
    reinforced_ids = {r["target"].id for r in reinforcement_trajectories}

    moves = []
    # sources = idle low-pressure planets, richest first (most to forward).
    for src in sorted(mine, key=lambda p: p.ships, reverse=True):
        if src.id in exhausted_planets_id or src.id in reinforced_ids:
            continue
        avail = src.ships
        if src.id in under_attack:
            enemy_ships = sum(att["fleet"].ships for att in under_attack[src.id]["fleets"])
            avail = max(0, avail - enemy_ships)
        if avail < MIN_SHIPS_MINE_ATTACK:
            continue

        src_pres = pressure.get(src.id, 0.0)
        best = None
        best_gap = REGROUP_PRESSURE_DELTA_MIN
        for dst in mine:
            if dst.id == src.id:
                continue
            gap = pressure.get(dst.id, 0.0) - src_pres
            if gap <= best_gap:
                continue

            if dst.id in moving_planets:
                angle, arrive_tick = find_angle_to_moving_planet(src, dst, avail, vel)
                if angle is None or arrive_tick is None:
                    continue
            else:
                angle = calculate_angle(src, dst)
                fs = _fleet_speed(avail)
                dist = math.sqrt((dst.x - src.x) ** 2 + (dst.y - src.y) ** 2)
                arrive_tick = math.floor(dist / fs)
                if sun_collision(src, fs, angle):
                    continue
            if arrive_tick > max_time:
                continue

            best = (dst, angle, arrive_tick)
            best_gap = gap

        if best is None:
            continue
        dst, angle, arrive_tick = best
...
def agent(obs):
    ...
    # --- logistics / regroup layer: marshal idle garrisons toward the front ---
    moves += plan_regroup(
        lobs, under_attack, exhausted_planets_id, num_players, obs.angular_velocity
    )
