...
def agent(obs):
    ...
    # Observed opp aggression: count enemy fleets and total ships in flight.
    enemy_fleet_count = sum(1 for f in raw_fleets if f[1] != player and f[1] != -1)
    enemy_total_ships = sum(f[6] for f in raw_fleets if f[1] != player and f[1] != -1)
    # If opp is launching a lot, they're aggressive — relax our sim filter so
    # we keep up. If opp is passive, stay selective (sim filter strict).
    # Scale: aggression=0 (no enemy fleets) -> TOL=0 (strict). aggression=1 (5+
    # fleets) -> TOL=80 (loose).
    aggression = min(1.0, enemy_fleet_count / 5.0)
    TOL = 80.0 * aggression
    ...
    for key in group_keys:
        ...
        if tid in targeted:
            continue
        best_variant = None
        best_eval_for_group = current_best - TOL  # adaptive tolerance
        ...
        if best_variant is not None:
            ...
            current_best = max(current_best, best_eval_for_group)
            used[mid] += ships
            targeted.add(tid)
