...
def agent(obs):
    ...
    # 2P: ≥6 small enemy fleets = swarm. 4P: ≥12 (more baseline since 3 opps).
    swarm_threshold = 12 if num_players == 4 else 6
    if n_small >= swarm_threshold:
        swarm_active = True
        # In 4P soften less aggressively (already at 0.25 floor)
        delta = 0.08 if num_players == 4 else 0.12
        shot_threshold = max(0.12, shot_threshold - delta)
    ...
    if _SHOT_TREES is not None:
        ...
        for c in candidates:
            ...
            if p_success >= shot_threshold:
                filtered.append(c)
            elif (swarm_active and ships <= 12 and eta <= 10
                  and p_success >= 0.18):
                filtered.append(c)
        candidates = filtered
