...
def agent(obs):
    ...
    # v68: 4P averages 2 sim rollouts (none + starter) for robust eval.
    # Single-model rollouts have noise; averaging may reduce false rejects.
    sim_state_base = sim.make_state_from_obs(obs)
    def _eval_avg(actions):
        if num_players == 4:
            s1 = sim.clone(sim_state_base)
            e1 = _simulate(s1, actions, [], SIM_HORIZON, player,
                            num_players=num_players, opp_model="none")
            s2 = sim.clone(sim_state_base)
            e2 = _simulate(s2, actions, [], SIM_HORIZON, player,
                            num_players=num_players, opp_model="starter")
            return (e1 + e2) / 2
        else:
            s = sim.clone(sim_state_base)
            return _simulate(s, actions, [], SIM_HORIZON, player,
                              num_players=num_players, opp_model="starter")
    baseline_eval = _eval_avg([])
    ...
    for key in group_keys:
        ...
        for score, _mid, _tid, ships, angle, T in grouped[key]:
            ...
            new_eval = _eval_avg(trial_moves)
