...
def agent(obs):
    ...
    # Third pass: multi-source split-attack on unclaimed targets.
    # For each unclaimed target where pooled spare ships can capture it,
    # try a combined attack from up to 3 mines. Sim validates the combo.
    spent_per_mine = {p.id: 0 for p in my_planets}
    for mv in accepted:
        spent_per_mine[mv[0]] += mv[2]
    for tgt in targets:
        if tgt.id in targeted:
            continue
        # Find mines with spare capacity that can reach this target
        contributors = []
        for mine in my_planets:
            spare = max_launch[mine.id] - spent_per_mine[mine.id]
            if spare < 4:
                continue
            dist = math.hypot(tgt.x - mine.x, tgt.y - mine.y)
            T_est = max(1, math.ceil(dist / _speed(spare)))
            pos = _predict_position(tgt.id, (tgt.x, tgt.y), initial_by_id,
                                      angular_velocity, step_now,
                                      step_now + T_est, comet_groups)
            if pos is None:
                continue
            angle = _safe_launch_angle((mine.x, mine.y), pos, tgt.radius)
            if angle is None:
                continue
            contributors.append((dist, mine.id, spare, angle, T_est))
        if len(contributors) < 2:
            continue
        contributors.sort()  # closest first
        # Need enough pooled to overcome target growth + initial garrison
        # Account for production growth assuming farthest arrival window
        max_T = max(c[4] for c in contributors[:3])
        prod = tgt.production if tgt.owner != -1 else 0
        need_total = tgt.ships + prod * max_T + 1
        pool_total = sum(c[2] for c in contributors[:3])
        if pool_total < need_total + 2:
            continue
        # Distribute ships proportionally (cap at need_total + margin)
        moves_combo = []
        remaining = need_total + 5
        for dist, mid, spare, angle, T_est in contributors[:3]:
            send = min(spare, remaining)
            if send < 3:
                continue
            moves_combo.append([mid, angle, int(send)])
            remaining -= send
            if remaining <= 0:
                break
        if not moves_combo:
            continue
        # Sim-validate combined attack
        trial = accepted + moves_combo
        trial_state = sim.clone(sim_state_base)
        new_eval = _simulate(trial_state, trial, [], SIM_HORIZON, player,
                              num_players=num_players, opp_model="starter")
        if new_eval > current_best:
            accepted = trial
            current_best = new_eval
            for mv in moves_combo:
                spent_per_mine[mv[0]] += mv[2]
            targeted.add(tgt.id)
