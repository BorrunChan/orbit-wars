...
SUN_BUFFER = 0.6
EPISODE_STEPS = 500
SIM_HORIZON = 6     # turns to look ahead
DEFENSE_HORIZON = 40
...
def agent(obs):
    ...
    for mine in my_planets:
        ...
        for tgt in targets:
            ...
            candidates.append((base_value / (min_ships + 0.3 * T),
                                mine.id, tgt.id, min_ships, angle, T))
            # Variant 2: buffered capture (~ +5 or +50% whichever is smaller)
            buf = min(int(min_ships * 0.25) + 5, avail - min_ships)
            if buf > 0:
                ...
                candidates.append((base_value / (big_ships + 0.3 * T) * 0.95,
