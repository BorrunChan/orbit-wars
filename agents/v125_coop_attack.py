"""
Orbit Wars - Agent v125 (coop attack via relaxed targeted lock).

Based on v124. Single behavioral change: in 2P, the per-target single-attack
lock in the greedy sim loop becomes a 2-source limit. v124 sets
`targeted.add(tid)` after the first accepted attack and skips any further
group with the same target; v125 replaces this with `targeted_count` and
allows up to 2 sources per target in 2P (4P stays at 1, preserving v124).

Differs from v105's failed coop attempt: v105 pre-computed coop splits via
`_capture_cost(combined)` on the 2 nearest mines and then fired both, which
caused wave-spacing mismatches (different mine→target distances → first
wave captures briefly, second arrives after recapture or hits own planet).
v125 makes no pre-computed split: the second source only emerges if the
greedy sim loop finds that adding it strictly improves eval over the state
with the first launch already applied. Time-mismatched second waves get
rejected naturally by sim. K (candidate cap) bumped 15→20 in 2P so a
second-source candidate for the same target survives the truncation.
"""
...
def agent(obs):
    ...
    # v125: per-target source count replaces the binary `targeted` lock.
    # 2P: up to 2 sources per target (sim-mediated coop); 4P: 1 (= v124).
    targeted_count = {}
    target_limit = 2 if num_players == 2 else 1
    ...
    # v125: bump K to 20 in 2P so a same-target second-source candidate
    # has room above the truncation line.
    K_cap = 20 if num_players == 2 else 15
    ...
    for key in group_keys:
        mid, tid = key
        if targeted_count.get(tid, 0) >= target_limit:
            continue
        best_variant = None
        ...
        if best_variant is not None:
            ...
            used[mid] += ships
            targeted_count[tid] = targeted_count.get(tid, 0) + 1
